...
4. **设置与输入体验优化**：商店设置页在软键盘弹出时会更好地滚动到当前输入框。
5. **夜间模式跟随系统逻辑更稳**：修复跟随系统与手动切换之间的状态竞争，减少主题状态不同步的问题。
6. **URL Scheme API 能力补充**：新增/编辑任务支持 `no_deadline` 无期限语义，计数任务相关 API 支持实时结算开关。
7. **利息历史记录信息更完整**：ATM 和信贷利息记录现在会显示本金和计息天数，方便核对利息来源。
