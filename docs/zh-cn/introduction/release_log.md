...
3. **设置与输入体验优化**：商店设置页在软键盘弹出时会更好地滚动到当前输入框；历史补录的计数输入框聚焦后会自动全选。
4. **夜间模式跟随系统逻辑更稳**：修复跟随系统与手动切换之间的状态竞争，减少主题状态不同步的问题。
5. **URL Scheme API 能力补充**：新增/编辑任务支持 `no_deadline` 无期限语义，计数任务相关 API 支持实时结算开关。
