...
> [!TIP]
> 如果你通过 Google Play Store 购买《人升》，短时间内卸载通常会触发 Google Play Store 自动退款。
...
- 多端清单互相同步的商务型需求：《人升》是一款离线应用，它目前不太适合多端、多设备同步使用。如有该需求，你可以尝试使用《Microsoft To Do》等商务型清单应用。
- 日程事件管理：《人升》并不负责日程事件的管理，也不会提供周视图、甘特图这种日历软件的功能。如有该需求，你可以尝试使用《TickTick》、《Google Calendar》、手机自带的日历应用等日程管理应用。
...
由于目前 Android 权限收紧很严重，各家机型也有着不同的限制。为了你能够正常使用应用的各项机制，请进行兼容性配置。
...
>
> 使用**扭蛋 + 合成**组合使用，你就能模拟简单的钓鱼系统、打怪探索体系：
>
> 渔具 + 消耗鱼料 = 钓鱼成果宝箱...
>
...
> 扭蛋是基于真实概率计算的，不会出现游戏里的伪随机浮动效果。**建议将概率设置得高一些。**
...
比如用来实现 `多货币`、`多物品` 奖励，又或者搭配上 `扭蛋` 实现复杂的 `集五福`、`钓鱼`、`集卡升级`。

如：
- `鱼线` + `鱼饵` + `钓鱼地点` —> `🐟扭蛋`
- `上锁的箱子`（每日签到得到） + `钥匙`（完成任务概率获得） -> `奖励扭蛋`
- ...
...
##### 4. 集五福（葫芦娃救爷爷）
你可以使用 `扭蛋` 和 `合成` 系统实现该机制。

<br/>

简言之就是：
`完成任务获得碎片扭蛋` -> `随机获得一种碎片` -> `集齐所有碎片` -> `合成特殊奖励`
...
这类成就在创建时就可以设定好 `解锁条件`，应用就会自动跟踪并计算进度，在你达成目标时提示成就已解锁。
