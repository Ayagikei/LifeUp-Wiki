...
> [!NOTE]
> 我们正在发布《人升》v1.106.0 以及《云人升》v3.0.0。由于各平台审核时间不一，文档可能已提前更新，App 实际上线时间可能会有所延误。
...
本文的 API 参数和定义基于 v1.106.0 版本编写。

使用 API 前，建议将应用升级到 v1.106.0 版本，如果没法检测到更新，请切换更新渠道到【会员内测-尝鲜版】。
...
当接口支持通过名称查找任务、商品或子任务时，会先尝试忽略大小写的完整名称匹配；如果没有完整匹配，才会回退到包含关系的模糊匹配。

例如同时存在「金币」和「金币箱」时，传入 `name=金币` 会优先匹配「金币」，而不是较早出现在搜索结果中的「金币箱」。

如果同名或近似名称可能造成歧义，仍建议优先传入对应的 id。
...
`purchase_limit` 是一个 JSON 数组，其中每个对象代表一条限制规则。

| 字段 | 含义 | 类型 | 是否必须 | 备注 |
| ---- | ---- | ---- | -------- | ---- |
| limitType | 限制类型 | 数字 | 是 | 见下方类型表 |
| limitNumber | 主要数值 | 数字 | 否* | 用于次数/区间类规则 |
| maxNumber | 区间上限 | 数字 | 否 | 用于属性等级区间、拥有物品数量区间 |
| limitId | 关联目标 ID | 数字 | 否* | 属性 / 物品 / 任务 / 任务周期 / 成就类规则需要 |
| extendInfo | 扩展信息 | 字符串 | 否 | 用于时间类规则；字段值本身是一个 JSON 字符串 |

**类型定义**

| limitType | 含义 | 必填字段 / 说明 |
| --------- | ---- | --------------- |
| 0 | 每日周期数量限制 | `limitNumber`：每天最多次数 |
| 1 | 每周周期数量限制 | `limitNumber`：每周最多次数 |
| 2 | 每月周期数量限制 | `limitNumber`：每月最多次数 |
| 3 | 每年周期数量限制 | `limitNumber`：每年最多次数 |
| 10 | 属性等级限制 | `limitId`：属性 ID<br/>`limitNumber`：最低等级<br/>`maxNumber`：可选，最高等级 |
| 20 | 每日时间段限制 | `extendInfo`：`{"startMinuteOfDay":540,"endMinuteOfDay":1320}` |
| 21 | 每周几限制 | `extendInfo`：`{"weekdays":[1,2,3,4,5]}`<br/>周一 = 1，周日 = 7 |
| 22 | 绝对时间段限制 | `extendInfo`：`{"startMillis":1710000000000,"endMillis":1710086400000}` |
| 23 | 月份限制 | `extendInfo`：`{"months":[1,6,12]}` |
| 24 | 每月日期限制 | `extendInfo`：`{"daysOfMonth":[1,15,31]}` |
| 30 | 拥有某物品数量限制 | `limitId`：目标物品 ID<br/>`limitNumber`：最小拥有数量<br/>`maxNumber`：可选，最大拥有数量 |
| 31 | 完成某任务限制 | `limitId`：任务 ID |
| 32 | 解锁某成就限制 | `limitId`：成就 ID |
| 33 | 按任务周期完成限制 | `limitId`：重复任务组 ID（`groupId`）<br/>按该任务组最新已开始周期是否完成来判定 |

**说明**

- 为兼容旧写法，可以不传 `maxNumber` 与 `extendInfo`。
- `limitType=33` 时，`limitId` 不是任务 ID，而是重复任务的 `groupId`。
- `extendInfo` 本身是字符串字段，因此通过 URL 调用 API 时，内部 JSON 字符串通常还需要额外转义/编码一次。
...
| number    | 奖励数量         | 大于 0 的数字                          | 1           | 是       | 如果是金币，取值最大为999999<br/>如果是经验值，取值最大为99999<br/>如果是商品，取值最大为999999（超出按上限发放）<br/>开启「设置 → 实验 → 减少应用限制」后，金币/商品上限为999999999，经验值上限为99999999 |
...
| weekdays        | 每周几         | `1,3,5` 或 `none` | 1,3,5     | 否       | v1.106.0+；1=周一 … 7=周日，列出**要重复**的星期。出现时 frequency 须省略或为 1。add 不接受 `none`；edit 传 `none` 清回每日。七天都给时按每日处理 |
...
| frequency       | 重复频次       | 整数               | 0         | 否       | 默认为 0（单次）<br/>0 - 单次<br/>1 - 每日<br/>N (N>1) - 每 N 日<br/>-1 - 无限<br/>-3 - 艾宾浩斯（需v1.99.1）<br/>-4 - 每月<br/>-5 - 每年 |
| weekdays        | 每周几         | `1,3,5` 或 `none` | 1,3,5     | 否       | v1.106.0+；与 add_task 相同。省略则不改；`none` 清回每日；改 frequency 为非 1 时会清掉每周几 |
| importance         | 重要程度       | [1, 4]            | 1        | 否       | 默认为 1                       |
...
| weekdays | 每周几 | `1,3,5` | 1,3,5 | 否 | v1.106.0+；与 add_task 相同，create 不接受 `none` |
...
**示例：**[lifeup://api/add_item?name=休息10分钟&desc=去好好休息一小段时间吧！&price=10&action_text=休息&icon=☕](lifeup://api/add_item?name=休息10分钟&desc=去好好休息一小段时间吧！&price=10&action_text=休息&icon=☕)

| 参数             | 含义           | 取值                | 示例         | 是否必须 | 备注                           |
| --------------- | -------------- | ------------------ | ------------ | -------- | ------------------------------ |
| name            | 商品名称       | 任意文本           | 休息10分钟    | 是      |                                |
| desc            | 描述           | 任意文本           | 休息一下      | 否       |                               |
| icon            | 图标           | emoji、http(s) URL 或内置样例名 | ☕ | 否 | 存储为 `emoji_*.webp`、`lifeup_sample_*` 或 URL。商品名称里的 emoji 不会自动成为图标。 |
...
| set_icon         | 修改图标       | emoji、http(s) URL 或内置样例名 | ☕ | 否 | 与 `icon` 相同。不支持的值返回 `unsupported_parameter`。 |
...
### 技能组管理 :id=skill_group_management
...
| frequency      | 重复频率         | 数字      | -    | 是       | -                              |
| weekdays       | 每周几           | 文本      | 1,3,5 | 否      | v1.106.0+；非每周几为空字符串。1=周一 … 7=周日 |
| exp            | 经验值奖励       | 数字      | -    | 是       | -                             |
