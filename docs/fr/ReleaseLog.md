...
1. **Serveur MCP officiel et Skills (LifeUp Cloud requis)** : les agents IA peuvent lire et agir sur LifeUp (ex. trier le backlog, analyser pomodoro/focus/pièces/XP, concevoir des tâches quotidiennes, monter un système gamifié). **Paramètres → Labo**, ou [documentation MCP](https://wiki.lifeupapp.fun/fr/index.html#/guide/api_mcp).
2. **URL Scheme / API tâches : paramètre `weekdays`** : créer ou modifier des répétitions hebdomadaires (1=lun … 7=dim) ; les requêtes renvoient aussi ce champ.

**♻️ Optimisation**

1. **URL Scheme / API élargis pour MCP et automatisation** : complétion d’achievements, listes masquées, journaux pièces/XP/inventaire, pas, courbe de niveaux et statistiques, événements broadcast, boutique/loot box/conditions d’achievements, etc.
2. **Validation API et messages d’erreur plus clairs** : erreurs explicites au lieu de succès silencieux ; contrôles renforcés (génération de niveaux, plages horaires, facteurs de pénalité).
3. **Couleurs du calendrier alignées sur le thème** : jour sélectionné, anneau de progression, aujourd’hui et marqueurs de la vue année.
4. **Les lectures query et ContentProvider pour boutique et achievements renvoient les champs d’écriture** (couleurs, libellés d’action, retrait/désactivation d’usage, etc.), alignés sur les API d’écriture pour l’automatisation et MCP.
5. **Lors de la mise à jour BD, seuls les groupes de tâches encore référencés sont réordonnés**, évitant les blocages dus aux groupes orphelins.
6. **Requêtes d’effets d’articles en arrière-plan avec chargement par lot** pour des parcours API/UI plus fluides.

**🐛 Corrections**

1. **Paliers de couleur incorrects sur la heatmap de comptage dans le détail de tâche.**
2. **Attributs personnalisés dont l’ordre de groupe pouvait se décaler après un appui long sans glisser.**
3. **Sélecteur de lien d’article qui se fermait en quittant la configuration du widget inventaire.**
4. **Widget inventaire ne se rafraîchissait pas après changement de quantité via API.**
5. **Copies en retard de tâches uniques non supprimées après complétion de rattrapage.**
6. **Écran de repos Pomodoro ne se rafraîchissait pas immédiatement après changement de tâche liée.**
7. **Progression d’achievement non mise à jour après usage automatique d’un article.**
8. **Plantages possibles en rouvrant rapidement certaines pages.**
9. **Retour arrière défaillant sur certaines pages, et ralentissements ou plantages occasionnels à l’usage.**

**v1.105.5 (2026/09/01)**

**🐛 Corrections**

1. **Correction d'un problème où des données de Tâches anormales pouvaient s'accumuler dans certains cas** : Cela pouvait rendre les opérations sur les Tâches, comme le tri ou la suppression, lentes ou sans réponse. La mise à jour nettoie automatiquement les données invalides concernées et améliore les performances avec de longs historiques de Tâches.
...
1. **Les API HTTP en lecture seule prennent en charge ETag** : la réponse inclut un `ETag`. Avec `If-None-Match`, si les données n’ont pas changé, le serveur renvoie 304 et le téléchargement est évité. Le client MCP officiel réutilise de la même façon les lectures inchangées.

**v3.0.1 (2026/09/13)**

**✨ Fonctionnalités**

1. **Les API de lecture HTTP/MCP exposent plus de champs boutique et succès** : couleur du titre, texte d’action, retrait/désactivation d’usage, limites d’achat/usage et portée ; couleur du succès. Nécessite LifeUp **1.106.0+**.

**♻️ Optimisation**

1. **Push WebSocket d’événements activé par défaut** (désactivable dans Avancé) ; le polling HTTP `GET /events` reste disponible.

**v3.0.0 (2026/08/28)**

**ℹ️ Important**

1. **LifeUp 1.106.0+ requis** : journaux, statistiques, courbe de niveau, événements broadcast et API associées nécessitent la version LifeUp correspondante.

**✨ Fonctionnalités**

1. **Pont MCP & Skills** : Cursor, Claude et clients IA similaires se connectent en LAN pour lire et agir sur LifeUp. Voir la [documentation MCP](https://wiki.lifeupapp.fun/fr/index.html#/guide/api_mcp).
2. **Scanner les QR codes lifeup:// API** : aide à côté de Scanner.
3. **Raccourcis lifeupcloud://** : carte d’état avec liens `start` / `stop` (Tasker, etc.).
4. **API d’événements broadcast** : HTTP/WS `/events` reçoit les broadcasts LifeUp (activer **Réglages → Labs → Mode développeur → Broadcast events** dans LifeUp ; Avancé dans Cloud affiche l’état).
5. **API HTTP de lecture élargies** : journaux pièces/inventaire/XP, pas, statistiques, courbe de niveau, groupes de compétences, conditions de succès, etc. ; listes de catégories avec `include_hidden`.

**♻️ Optimisation**

1. **Découverte LAN améliorée** : mDNS annonce le port HTTP réel.
2. **Android 16** ; 14 langues dans l’app.

**🐛 Corrections**

1. **Correction des problèmes d’abonnement aux événements et de redémarrage du service**.
