...
1. **Официальный MCP‑сервер и Skills (нужен LifeUp Cloud)**: AI‑агенты могут не только читать, но и управлять LifeUp (сортировка backlog, анализ pomodoro/фокуса/монет/опыта, проектирование задач, gamification‑система). **Настройки → Лаборатория** или [документация MCP](https://wiki.lifeupapp.fun/ru/index.html#/guide/api_mcp).
2. **URL Scheme / API задач: параметр `weekdays`**: повтор по дням недели (1=пн … 7=вс); поле также возвращается в запросах.

**♻️ Оптимизация**

1. **Расширены URL Scheme / API для MCP и автоматизации**: завершение достижений, скрытые списки, журналы монет/опыта/инвентаря, шаги, кривая уровней и статистика, broadcast‑события, магазин/лут‑боксы/условия достижений и др.
2. **Более понятная валидация API и ошибки**: явные ошибки вместо «тихого успеха»; усилены проверки генерации уровней, диапазонов времени и штрафных коэффициентов.
3. **Цвета календаря следуют теме приложения**: выбранный день, кольцо прогресса, «сегодня» и метки годового вида.
4. **Чтение магазина и достижений через query и ContentProvider теперь возвращает поля записи** (цвета, подписи действий, снятие с продажи/отключение использования и т. д.), согласованно с API записи для автоматизации и MCP.
5. **При обновлении БД переупорядочиваются только группы задач, на которые ещё есть ссылки**, что предотвращает долгие зависания из‑за «осиротевших» групп.
6. **Запросы эффектов предметов выполняются в фоне с пакетной загрузкой** для более плавных API и UI.

**🐛 Исправления**

1. **Исправлены неточные уровни цвета на heatmap подсчёта в деталях задачи.**
2. **Исправлен сбой порядка групп пользовательских атрибутов после долгого нажатия без перетаскивания.**
3. **Исправлено закрытие выбора ссылки на предмет при выходе из настройки виджета инвентаря.**
4. **Исправлено: виджет инвентаря не обновлялся после изменения количества через API.**
5. **Исправлено: просроченные копии разовых задач не удалялись после догоняющего завершения.**
6. **Исправлено: экран отдыха Pomodoro не обновлялся сразу после смены связанной задачи.**
7. **Исправлено: прогресс достижений не обновлялся после автоматического использования предмета.**
8. **Исправлены сбои при быстром повторном открытии некоторых экранов.**
9. **Исправлена неработающая кнопка «назад» на некоторых экранах, а также редкие подтормаживания или сбои при использовании.**

**v1.105.5 (2026/09/01)**

**🐛 Исправления ошибок**

1. **Исправлена проблема накопления аномальных данных Задач в некоторых случаях**: из-за этого операции с Задачами, такие как сортировка или удаление, могли работать медленно или не отвечать. Обновление автоматически очищает затронутые недействительные данные и улучшает производительность при больших историях Задач.
...
1. **HTTP API только для чтения поддерживают ETag**: ответ содержит `ETag`. С заголовком `If-None-Match`, если данные не изменились, сервер возвращает 304 и повторная загрузка не нужна. Официальный клиент MCP так же переиспользует неизменённые ответы.

**v3.0.1 (2026/09/13)**

**✨ Функции**

1. **HTTP/MCP read API теперь отдают больше полей магазина и достижений**: цвет заголовка, текст действия, снятие с витрины/запрет использования, лимиты покупки/использования и область; цвет достижения. Требуется LifeUp **1.106.0+**.

**♻️ Оптимизация**

1. **WebSocket push событий включён по умолчанию** (можно отключить в «Дополнительно»); HTTP `GET /events` по-прежнему доступен.

**v3.0.0 (2026/08/28)**

**ℹ️ Важно**

1. **Требуется LifeUp 1.106.0+**: журналы, статистика, кривая уровней, broadcast-события и связанные API нуждаются в соответствующей версии LifeUp.

**✨ Функции**

1. **Мост MCP & Skills**: Cursor, Claude и похожие AI-клиенты подключаются по LAN к Cloud для чтения и управления LifeUp. См. [документацию MCP](https://wiki.lifeupapp.fun/ru/index.html#/guide/api_mcp).
2. **Сканирование QR lifeup:// API**: справка рядом со «Сканировать».
3. **Ярлыки lifeupcloud://**: карточка статуса с ссылками `start` / `stop` (Tasker и т. п.).
4. **API broadcast-событий**: HTTP/WS `/events` принимает broadcast LifeUp (включите **Настройки → Лаборатория → Режим разработчика → Broadcast events** в LifeUp; «Дополнительно» в Cloud показывает статус).
5. **Расширенные HTTP read API**: журналы монет/инвентаря/опыта, шаги, статистика, кривая уровней, группы навыков, условия достижений и др.; списки категорий с `include_hidden`.

**♻️ Оптимизация**

1. **Улучшенное LAN-обнаружение**: mDNS объявляет реальный HTTP-порт.
2. **Android 16**; 14 языков в приложении.

**🐛 Исправления**

1. **Исправлены проблемы подписки на события и перезапуска сервиса**.
