...
?> In the v1.90 version, `Rensheng` has opened a variety of functional interfaces, and any external application linkage is welcome. <br/>It also provides the "link" effect of commodities, and users can directly use commodities to call external applications or the interface of "Rensheng". <br/>This can make your `human level` get unlimited possibilities, but it also requires you to have a certain learning understanding and hands-on ability.
...
**If you have programming foundation,** you can try to develop web pages, Android applications and Rensheng linkage. This web page is a super simple example.

---

## Call external APP

If you want to call an external application in Rensheng, you only need to add a "link" effect to the product, and **enter a web page or application-specific link**, and then use the product to complete the call.
...
In theory, as long as the external APP opens the corresponding link, you can jump by using the product.

**You can search for the application link you need by searching `URL Schemes` on Baidu. **

**These links have nothing to do with "Rensheng", "Rensheng" only provides the jump function. **

**Therefore we do not and do not maintain a list of links. <del>But in the future, we will consider supporting the window sharing of this type of product. </del> (Showcase sharing is currently supported)**
...
- You need to call the "Rensheng" interface in "Rensheng", you only need to add a "link" effect to the product, and **enter a web page or application-specific link**, and then use the product to complete the call.
...
When you click the link below, "Rensheng" will pop up a prompt "You have learned to call!".
...
When you click the link below, you will get a gold coin in "Rensheng", and the reason for it is "Learn API Calls".
...
**This version of Wordle supports linkage with "Rensheng":**
...
The data storage in Rensheng basically has a unique id.

Some of the following interfaces support modifying **specified** products and completing **specified** tasks.

In order for "Rensheng" to find the corresponding data, you need to provide it with an id.
...
"Developer Mode", and then click on the product to view the details to view.
...
| skills | Skills (attributes) | Array of numbers greater than 0 | 1 | No | Available only when type is exp<br/>Supported arrays (eg &skills=1&skills=2&skills=3)<br/>For how to obtain, see above The article "Basic Knowledge - Rensheng Data ID" |
...
**Explanation: **Add a content to the default list (id is 0) with the content "This is an automatically added task", the remark is "Remark", the gold coin reward is random from 10 to 11, the experience value reward is 2048, the selected The skill ids are 1, 2, and 3 (generally corresponding to the first 3 built-in attributes), and the product reward is a fuzzy search for a "gold coin" product.
...
**Description:** Jump to a page in "Rensheng"
...
**Description:** To create a product, the icon only supports network addresses, and the use effect does not support customization.
...
**Explanation:** Create a product with the name of "Take a 10-minute break", the description as "Go and take a good rest for a short period of time!", and the operation copy as "Rest" with a price of 10 gold coins.

| Parameter | Meaning | Value | Example | Required | Remarks |
| ---------------- | ------------ | ---------------- | -- ---------- | -------- | -------------------- |
| name | Product name | Any text | 10 minute break | Yes | Product name |
...
**Description:** Various operations on the product with the specified id only support the [On Shelf] product.
...
| Parameter | Meaning | Value | Example | Required | Remarks |
| ----------------- | ------------------------- | ----- ----------------------------------- | ------------ | - ------- | ------------------------------------------------------ ------------------ |
| id | Product id | A number greater than 0 | 1 | No* |
...
"Rensheng" provides user intervention processing methods for parameters.
...
For example, after you set the API for price reduction of a certain product, you may want to allow the user to select the specified product when calling. ** instead of specifying the id when calling.

The following api can only reduce the price of the product with id 1 by 1 gold:
...
Of course, you can also add multiple links to a product to achieve this effect.

This callback is more used for:

A application -> Rensheng -> A application
...
If you are an application developer and support linkage with "Rensheng", interesting functions and mechanisms have been realized.
...
If you are a web developer and have developed web works that call the "Rensheng" API, you are welcome to share your works in the form of products within the app.
