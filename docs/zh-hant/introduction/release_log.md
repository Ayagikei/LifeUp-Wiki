...
3. **修復小部件摺疊過濾失效問題**：修復了智慧清單小部件「摺疊未開始事項」過濾不生效的問題。
4. **修復部分場景閃退問題**：修復了在少數情況下因儲存狀態過大導致的閃退。
5. **修復日曆查看未來任務詳情問題**：修復了從日曆視圖開啟未來重複任務詳情失敗的問題。
6. **修復記憶法重複任務異常**：修復了記憶法任務在少數情況下階段異常/重複生成的問題，並增加上限兜底。
...
2. **修復番茄鐘計時異常變長問題**：修復了在未忽略電池最佳化時，因系統休眠或排程凍結導致計時時長異常增加的問題。
