...
4. **設定與輸入體驗最佳化**：商店設定頁在軟鍵盤彈出時會更好地滾動到當前輸入框。
5. **夜間模式跟隨系統邏輯更穩**：修復跟隨系統與手動切換之間的狀態競爭，減少主題狀態不同步的問題。
6. **URL Scheme API 能力補充**：新增/編輯任務支援 `no_deadline` 無期限語義，計數任務相關 API 支援實時結算開關。
7. **利息歷史記錄資訊更完整**：ATM 和信貸利息記錄現在會顯示本金和計息天數，方便覈對利息來源。
