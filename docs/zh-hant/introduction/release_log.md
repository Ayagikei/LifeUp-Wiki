...
| 平臺         | 版本號     | 更新日期          |
| ------------ |---------|---------------|
| 人升-Android | v1.101.8 | 2026/01/12    |
...
2. **商店物品使用效果升級**：新增隨機經驗值、庫存變更、播放音效、倒計時等效果，並最佳化設定流程。
3. **新增計時任務**：建立任務時可選擇「計時任務」，設定預期專注時長，達成目標後可完成並獲得獎勵。
4. **更靈活的週起始日**：支援選擇週一 / 週六 / 週日，日曆與統計等頁面同步生效。
5. **歡迎頁可跳過新手指引**：首次安裝可直接跳過新手引導，更快進入使用。
6. **圖示選擇支援直接輸入 Emoji**：在圖示選擇器中可輸入 Emoji（含組合 Emoji）快速生成圖示。
7. **日曆可查看未來重複任務詳情**：從日曆點選未來日期的重複任務可正常進入詳情頁查看資訊/統計等內容。
8. **桌面小部件任務過濾增強**：新增「摺疊未開始事項」過濾，減少清單干擾。
9. **進階使用者：API 能力增強**：URL Scheme API 新增/增強任務模板管理、重複結束條件、合成篩選跳轉等能力。

**♻️ 最佳化**

1. **效能與流暢度提升**：最佳化資料讀寫與索引策略，任務清單、倉庫歷史、統計等頁面在大資料量下更順滑。
2. **使用效果設定更順手**：最佳化使用效果選擇與展示互動，調整彈窗佈局與圖示表現。
3. **多語言體驗完善**：補全缺失的本地化文字。

**🐛 修復**

1. **修復倒計時效果編輯重置問題**：修復了編輯倒計時效果後確認會被重置為 1 的問題。
2. **修復音效檔案遺失提示不明確問題**：音效檔案不存在時會給出清晰提示，避免使用商品時出現異常體驗。
3. **修復團隊任務商品獎勵自動使用失效問題**：修復了團隊任務領取商品獎勵並勾選自動使用後，自動使用無法觸發的問題。
4. **修復開箱/簡易合成後不彈記錄感想問題**：修復了在開箱或簡易合成後，若同時存在「記錄感想」效果，結束後不會彈出記錄感想的問題。
5. **修復番茄鐘手動完成不彈結算彈窗**：修復了番茄鐘頁面手動完成任務時，獎勵/感想等結算彈窗未出現的問題，並恢復長按任務名稱完成任務的入口。
6. **修復小部件摺疊過濾失效問題**：修復了智慧清單小部件「摺疊未開始事項」過濾不生效的問題。
7. **修復部分場景閃退問題**：修復了在少數情況下因儲存狀態過大導致的閃退。
8. **修復日曆查看未來任務詳情問題**：修復了從日曆視圖開啟未來重複任務詳情失敗的問題。
9. **修復記憶法重複任務異常**：修復了記憶法任務在少數情況下階段異常/重複生成的問題，並增加上限兜底。
