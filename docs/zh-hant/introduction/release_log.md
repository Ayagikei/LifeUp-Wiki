...
2. **最佳化獎勵兌換中的屬性選擇邏輯**：在「堅持天數獎勵」、「步數獎勵」及「獲讚獎勵」設置中，最佳化了未選擇屬性時的處理，現已支援取消選擇屬性，並修復了經驗值獎勵發放異常的問題。
3. **性能配置最佳化**：優化了內部配置項，以提升 APP 的運行性能。

**🐛 修復**

1. **修復統計查詢與圖表顯示異常**：修復了任務完成時間統計的相關查詢條件，解決了圖表數據展示不準確的問題。
2. **修復桌面小部件設置彈窗交互**：修復了小部件任務過濾設置彈窗中，缺少「確認」按鈕的問題。
3. **修復任務計數刷新延遲**：修復了在任務詳情頁面更新計數後，首頁任務計數未同步刷新的問題。
4. **修復感想 API 處理邏輯**：修復了感想 API 在處理「物品使用」類型時邏輯不正確的問題。
