...
| 平臺         | 版本號     | 更新日期          |
| ------------ |---------|---------------|
| 人升-Android | v1.106.0 | 2026/09/23    |
| 人升-iOS     | 請檢視[ulives](features/ulives)  | 🎉精神續作v1.x已上線 |
| 人升-桌面端  | v1.2.0  | 2025/01/01    |
| 雲人升       | v3.0.2  | 2026/09/24    |
...
1. **官方 MCP 伺服器與 Skills（需搭配 LifeUp Cloud）**：AI Agent 不只能讀取資料，還能理解並操作人升（例如整理今日待辦、分析番茄/專注/金幣/經驗、設計日常任務，或搭建完整遊戲化體系）。入口：**設定 → 實驗室**，或查閱 [MCP 文件](https://wiki.lifeupapp.fun/zh-hant/index.html#/guide/api_mcp)。
2. **任務 URL Scheme / API 新增 `weekdays` 參數**：可按「每週幾」建立或編輯重複任務（1=週一 … 7=週日），查詢結果也會回傳該欄位。

**♻️ 優化**

1. **大幅擴充 URL Scheme / API，便於 MCP 與自動化整合**：補完成就領獎、隱藏清單查詢、金幣/經驗/倉庫流水、步數記錄、等級曲線與統計資料、廣播事件、商店/開箱/成就解鎖條件等能力。
2. **API 校驗與錯誤提示更清晰**：無效或越界參數會回傳明確錯誤，不再出現「呼叫成功但什麼都沒發生」；部分邊界（如等級生成、時間範圍、懲罰係數等）校驗更嚴格。
3. **日曆頁元件顏色跟隨 App 主題色**：選中圓、完成進度環、今日數字與年檢視標記會與目前主題色保持一致。
4. **query 與 ContentProvider 讀取商品/成就時，會回傳顏色、操作文案、下架/禁用使用等已支援寫入的欄位**，與寫入類 API 對齊，便於自動化與 MCP 核對資料。
5. **App 升級時僅重排仍被任務引用的任務組**，避免歷史無效組資料拖慢升級甚至長時間無回應。
6. **商品效果相關查詢改為背景載入**，相關 API 與介面路徑更流暢。

**🐛 修復**

1. **修復任務詳情頁計數熱力圖顏色檔位不準確的問題**：低完成量更容易區分，超額完成仍保持較深顏色。
2. **修復自訂屬性長按後原地鬆手可能導致分組順序錯亂的問題**。
3. **修復設定倉庫小元件時，選擇商品連結彈窗會隨頁面一起關閉的問題**。
4. **修復透過 API 修改物品數量後，倉庫小元件不重新整理的問題**。
5. **修復補完成單次任務後，對應逾期副本未被刪除的問題**。
6. **修復番茄鐘休息階段更換關聯任務後，介面未立刻重新整理的問題**。
7. **修復自動使用物品後，相關成就進度未更新的問題**。
8. **修復部分頁面在快速重複開啟時可能當機的問題**。
9. **修復部分頁面返回失效，以及使用過程中偶發卡頓或當機的問題**。

**v1.105.5（2026/09/01）**

**🐛 修復**

1. **修復特定場景下任務資料異常積累的問題**：這類資料可能導致排序、刪除等任務操作卡頓或無響應。升級後會自動清理相關無效資料，並改善大量歷史任務下的操作流暢度。

**v1.105.4（2026/08/30）**

**♻️ 最佳化**

1. **最佳化日/周/月智慧清單在大量無期限任務下的載入效能。**

**🐛 修復**

1. **修復重設後的逾期單次任務未出現在日/周/月智慧清單中的問題。**
2. **修復任務詳情頁任務名稱過長時與上方清單名稱重疊的問題。**
...
1. **修復將團隊任務在本地轉換為計時任務後，完成任務表現異常的問題。**

**v1.105.2（2026/08/24）**

**🐛 修復**

1. **修復任務選擇器在智慧清單下列表為空的問題。**
2. **修復在智慧清單中搜索時，會列出當前清單以外任務的問題。**

**v1.105.1（2026/08/19）**

**✨ 特性**

1. **自定義音效支援按場景單獨禁用**：某個提示音可以靜音，不影響其他場景；試聽仍可播放以便確認。

**♻️ 最佳化**

1. **無 Google Play 服務時備份入口提示更清晰**：無法使用 Google Drive 時會說明原因，並提示改用本地檔案、Dropbox 或 WebDAV。

**🐛 修復**

1. **修復日曆「已結束」頁面點選撤銷沒反應的問題。**
2. **修復編輯商品時購買/使用限制可能丟失或顯示為空的問題。**

**v1.105.0（2026/08/04）**

**ℹ️ 重要說明**

1. **最低支援 Android 6.0**：為主動適配較新 Android 版本的系統特性和行為要求，本版本將最低支援版本從 Android 5.0 提升至 Android 6.0。因此 Android 5.x 使用者將無法安裝或升級本版本。

**✨ 特性**

1. **新增番茄鍾控制 URL Scheme API**：支援查詢狀態、選擇事項，以及啟動、暫停、跳過、放棄或結算番茄鍾/正計時會話。
2. **最佳化任務自定義排序機制**：引入新的排序機制，保證自定義順序在複製任務、完成任務、撤銷任務等特殊場景下保持穩定，避免任務順序異常變化。

**♻️ 最佳化**

1. **適配 Android 預測性返回**：任務、商品、合成和成就等編輯頁面支援系統預測性返回手勢。
2. **最佳化 Facebook 登入初始化**：改為點選登入時初始化 SDK，並改善異常狀態處理。

**🐛 修復**

1. **修復透過 URL Scheme 編輯事項時部分欄位可能被意外覆蓋的問題**：缺失欄位會保留原值，非法獎勵或關聯引數不會清空已有資料。
2. **修復特殊場景下正計時通知標題未跟隨當前事項更新的問題。**
3. **修復首次貼上十六進制顏色後 RGB 預覽不重新整理的問題。**
4. **修復合成和成就詳情頁選擇工具欄陰影缺失的問題。**

**v1.104.6（2026/07/19，Google Play）**

**🐛 修復**

1. **修復番茄鍾在應用或計時服務異常結束後恢復狀態不正確的問題**：有效的計時會話現在可以正常恢復，無效的舊狀態也會被清理。
2. **修復系統時區變化後任務時間調整可能反覆提示的問題**：確認調整後，任務時間會正常更新，後續不會重複提示。

**v1.104.5（2026/07/17）**

**🐛 修復**

1. **修復 v1.104.4 正式版進入番茄頁面時可能異常閃退的問題。**

**v1.104.4（2026/07/17）**

**✨ 特性**

1. **商品支援按屬性經驗值範圍設定購買/使用限制**：可設定經驗值上下限，控制商品能否購買、使用，或同時限制兩者。
2. **新增「每天累計完成 N 次任務」成就解鎖條件**：與已有的「每天完成 N 個不同任務」不同，該條件會統計當天每一次有效的任務完成記錄，包括同一任務的重複完成次數。

**♻️ 最佳化**

1. **重構番茄鍾結算與恢復流程**：倒計時狀態、專注記錄與獎勵結算現在使用統一流程；應用程序被終止後的恢復也更加可靠，減少專注時長缺失等邊界異常。如遇到非預期的行為變化，歡迎傳送郵件至 lifeup@ulives.io。
2. **最佳化合成配方的數量編輯流程**：點選已有素材或產物即可直接修改數量，無需重新選擇同一商品；需要時仍可重新選擇商品。
3. **首頁番茄鍾支援直接進入橫屏頁面**：橫向檢視和操作計時器更加方便。

**v1.104.3（2026/07/09）**

**✨ 特性**

1. **歡迎頁新增「快速設定」引導**：首次啟動新增第 5、6 頁，以手風琴形式集中完成通知許可權、提醒方式、介面風格（Material 2/3）、多視窗顯示等初始設定，上手更輕鬆。所有選項後續均可在設定中隨時調整。

**♻️ 最佳化**

1. **歡迎頁功能介紹文案最佳化**：第 1–4 頁介紹文案重寫，更清晰地傳達「自定任務→屬性成長→獎勵體系→連線世界」的產品價值。
2. **合成彈窗改為底部彈窗**：素材和結果改為縱向網格展示，視覺效果更直觀，互動更順手。
3. **最佳化任務列表切換清單時的資料載入速度**：切換清單、變更排序和分組設定時響應更快。

**🐛 修復**

1. **修復部分系統成就重複彈出多條「成就解鎖」通知的問題**。
2. **修復「每日完成 N 個不同任務」成就統計不準**：無限任務不再被錯誤忽略，同一任務當日多次完成只計為 1 個，與文案語義一致。
3. **修復任務列表右下角新增按鈕（+）在部分場景下位置漂移的問題**。
4. **修復智慧清單關閉「按清單分組」時未過濾已歸檔清單任務的問題**。

**v1.104.2（2026/07/03）**

**✨ 特性**

1. **統計頁「使用天數」改名為「相伴天數」**：支援在統計頁點選卡片自定義起始日期，相關成就條件描述也同步更新為「相伴」措辭。
2. **備份選單新增「備份並分享」選項**：可將備份檔案透過系統分享面板直接傳送至其他應用。
3. **開箱效果 API v2**：新增 `loot_box/v2` 路由，支援透過 `sub_amount` 精準定位條目、增刪條目，以及獨立控制數量與機率模式。

**♻️ 最佳化**

1. **URL Scheme 編輯商品/開箱/子任務時，優先精準匹配名稱**：匹配不到時再降級模糊匹配，避免誤編輯。
2. **英文版側邊欄「FAQ」更名為「Notice」**：中文原本即為「公告」，不受影響。
3. **最佳化多處按鈕點選水波紋圓角效果**：水波紋動畫現在會正確匹配控制元件的圓角形狀，不再溢位到圓角之外。

**🐛 修復**

1. **修復番茄鍾頁面新增/編輯記錄後番茄數不自動重新整理的問題**。
2. **修復新增番茄記錄後 toast 番茄數量提示偶發不出現的問題**。
3. **修復手動新增計時記錄時，番茄數總是按當前選中事項（而非記錄所選事項）計算的問題**：現在改為按記錄裏實際選擇的事項的專屬專注設定折算。如果你為不同事項設定了不同的專注時長，手動補錄的番茄數會更準確。
4. **修復無限任務歷史記錄中「完成次數」顯示異常**：改為按天序號顯示（當天完成第 N 次）。
5. **修復懲罰任務完成時激勵語不顯示的問題**：完成懲罰後現在會彈出激勵語。

**v1.104.1（2026/06/17）**

**✨ 特性**

1. **備份匯出選項增強**：手動備份時彈出底部選單，支援選擇是否包含媒體檔案、賬號敏感資訊（登入狀態、WebDAV 賬號密碼等）和 emoji 圖片，方便分享脫敏備份資料。自動備份區域也增加了對應的三項持久化開關。
2. **番茄鍾任務選擇器最佳化**：選擇番茄任務時，計時任務會優先顯示並標註當前專注進度（已專注時長/目標時長/百分比），並支援開關「優先顯示計時任務」，方便快速定位計時任務。
3. **關於頁重構**：重新調整關於頁面佈局為「連結」「反饋」「社群」「開發者」分割槽，新增網站、更新日誌、FAQ、API 文件等入口；簡中使用者增加騰訊頻道新增入口，海外使用者增加 GitHub Issues/Discussions 社群入口。
4. **新增移除粉絲功能**：在團隊成員頁支援移除粉絲。
5. **商品經驗屬性選擇器增強**：在商品經驗/效果輸入彈窗中增加屬性選擇器按鈕，支援按技能組快速篩選和批次多選屬性，大量屬性場景下操作更高效。

**♻️ 最佳化**

1. **任務詳情番茄統計支援切換時間顯示格式**：點選番茄統計區域可在「小時分鐘」「天時分」「總分鐘」三種格式間靈活切換。
2. **子任務完成態顯示最佳化**：子任務完成後現在會顯示刪除線，已完成和未完成的狀態區分更清晰。
3. **番茄鍾記錄排序修正**：修正番茄記錄的排序邏輯，按結束時間降序排列。

**🐛 修復**

1. **修復 ATM 取出按鈕多語言文案錯誤**：修正部分語言下 ATM 取出按鈕的動名詞誤用問題。
2. **修復未登入時商店物品分享行為異常**：修復未登入狀態下商店物品分享觸發異常表現的問題。
3. **修復顏色選擇回顯狀態異常**：修復顏色選擇對話方塊回顯時的狀態異常。

**v1.104.0（2026/05/23）**

**✨ 特性**

1. **計數任務支援按比例實時結算獎勵**：適合在一個週期內不固定時間、多次推進的任務，例如每週完成幾次的習慣；計數進度變化時，可按當前進度實時發放或回收經驗、金幣和物品獎勵，而不必等到最終完成時才統一結算。
2. **任務詳情頁歷史能力增強**：在歷史統計日曆中選擇日期後，可以檢視當日計數，並支援補錄、編輯單條歷史記錄或批次補錄歷史記錄。
3. **感想與商品聯動增強**：商品詳情中可直接檢視與該商品相關的感想，感想頁面也支援按商品篩選。
4. **子任務完成時間記錄**：完成子任務時會記錄對應的完成時間，併爲後續 API 和 LifeUp Cloud 同步、展示相關資料提供支援。

**♻️ 最佳化**

1. **商店商品隱藏篩選更細**：除售罄商品外，現在還可以隱藏禁用購買、購買受限或當前金幣不足的商品，商店小部件也會同步使用這些篩選規則。
2. **任務列表搜尋與重新整理更穩定**：普通清單搜尋可包含當前設定下可見的已完成任務；重複任務、本地重新整理和“全部”清單自定義排序在大量任務場景下更穩定。
3. **任務詳情頁歷史統計更貼合所選日期**：計數任務和無限任務會根據當天完成次數顯示不同深淺的貢獻圖顏色；歷史下方的本月、今年、全期和連續統計也會基於當前選中的時間點計算，而不再固定顯示當前日期的資料。
4. **按字母排序更符合直覺**：各模組的按字母排序現在會遵循數字自然排序邏輯，例如帶數字的名稱會按數值大小排序，而不是簡單按字元逐位比較。
5. **設定與輸入體驗最佳化**：商店設定頁在軟鍵盤彈出時會更好地滾動到當前輸入框。
6. **夜間模式跟隨系統邏輯更穩**：修復跟隨系統與手動切換之間的狀態競爭，減少主題狀態不同步的問題。
7. **URL Scheme API 能力補充**：新增/編輯任務支援 `no_deadline` 無期限語義，計數任務相關 API 支援實時結算開關。
8. **利息歷史記錄資訊更完整**：ATM 和信貸利息記錄現在會顯示本金和計息天數，方便覈對利息來源。

**🐛 修復**

1. **修復“我的”頁成就統計不一致的問題**：隱藏系統成就時，成就統計會與可見性設定保持一致。
2. **修復歷史記錄文案問題**：放棄歷史記錄不再錯誤顯示為逾期懲罰。
3. **修復 URL Scheme API 長文字問題**：技能、成就描述等長文字不再被過早截斷。

**v1.103.6（2026/05/10）**

**🐛 修復**

1. **修復點選「放棄」後，距離下次長時休息的間隔提示沒有重置回初始專注狀態的問題。**
2. **修復點選「放棄」後，追加專注計時器仍可能繼續顯示和計時的問題。**

**v1.103.5（2026/05/10）**

**🐛 修復**

1. **修復番茄鍾在正常工作計時結束後，長時休息提示沒有及時重新整理的問題。**
2. **修復未開啟過番茄設定時，番茄鍾可能顯示舊的 2 節長休間隔，而不是預設 4 節的問題。**

**v1.103.4（2026/05/05）**

**🐛 修復**

1. **修復了在凍結任務凍結期間，部分特殊場景下任務會異常中斷並消失的問題。**
...
1. **修復了修改物品購買或使用限制時，在部分語言或螢幕較小情況下輸入框顯示不出來的相容性問題。**
2. **修復了使用物品後錯誤觸發懲罰彈窗的問題。**

**v1.103.1（2026/04/25）**

**🐛 修復**

1. **修復了商品的使用效果會修改自己的數量時的異常行為**
2. **修復了部分線上採集到的崩潰和卡頓問題**
3. **修復了在編輯商品的[修改商品數量]效果時，編輯回填的數量不正確的問題**

**v1.103.0（2026/04/12）**

**✨ 特性**

1. **屬性支援子分類與快速移動**：可以更清晰地給屬性分組，並更方便地把屬性快速移到目標分組。
2. **屬性選擇互動更順手**：在任務編輯、獎勵相關流程等需要選擇屬性的場景裡，大量屬性時也更容易查詢和選擇。
3. **商品支援購買/使用限制**：限制可作用於購買、使用或同時作用於兩者，並支援時間、解鎖成就、完成任務、擁有物品數量、屬性等級區間等更豐富的條件。
4. **成就新增更多內建排序方式**：除自定義排序外，成就清單現已支援按字母順序、完成時間、建立時間等方式排序。
5. **成就支援快速移至子分類**：支援將單個或多個成就更高效地移動到目標子分類。
6. **逾期處理新增說明入口**：在逾期處理彈窗中增加了說明入口，也會更清晰地展示將事項改回完成時對應的獎勵變化與動畫。

**♻️ 最佳化**

1. **屬性分組展示更清晰**：狀態頁與屬性說明彈窗現在會以分組結構展示屬性，層級更清楚。
2. **屬性相關互動更統一**：屬性說明、屬性選擇等相關流程的展示與選擇體驗更加一致。
3. **商品限制編輯更易理解**：更豐富的限制類型現在更容易配置和檢視。
4. **多個頁面的狀態列與頂部欄顯示更穩定**：商店、世界、搜尋、狀態、成就、我的主頁、WebDAV 等頁面在滾動、夜間模式和 Material You 場景下的頂部顯示效果更穩定。
5. **狀態頁屬性佈局更適配長文字**：現在可以更好地支援更長的屬性名稱和等級展示，在窄屏或大字號下也更清晰。

**🐛 修復**

1. **修復副本 gid 變化問題**：修復了單次和無限任務逾期後，生成的副本 gid 會變化的問題。
2. **修復屬性選擇框無效隨機勾選問題**：修復了部分屬性選擇框會出現無作用的"隨機"勾選框的問題。
3. **修復獎勵橫幅位置異常**：修復了獎勵橫幅在部分場景下（特別是完成任務觸發），橫幅位置不正確、被異常遮擋或者位置跳動的問題。
4. **修復逾期處理獎勵預覽和動畫口徑可能不一致的問題**：在將逾期事項改回已完成時，經驗值、金幣和物品的展示會更準確，不再混入容易引起誤解的數值。
5. **修復狀態頁屬性等級在部分場景下容易換行錯位的問題**：長屬性名或較長等級文字下，佈局顯示更穩定。
6. **修復合成列表條目較少時空白區域無法左右滑動切換的問題**。
7. **修復領取成就獎勵崩潰問題**：修復了在某些特殊場景下，領取成就獎勵可能會崩潰的問題。

**v1.102.11 (2026/04/02)**

**🐛 修復**

1. **修復首頁計數任務結算偶發失效與重新整理異常的問題**。

**v1.102.10 (2026/03/24)**

**🐛 修復**

1. **修復了檢視被隱藏的合成配方清單時，沒法長按編輯或者刪除配方的問題**。

**v1.102.9 (2026/03/23)**

**♻️ 最佳化**

1. **底部導航欄模組禁用策略調整**：在互動層面上，不再允許使用者禁用全部底部導航欄模組。

**🐛 修復**

1. **修復啟動崩潰問題**：修復了將底部導航欄模組全部禁用後，可能導致 App 啟動崩潰的問題。

**v1.102.8 (2026/03/23)**

**✨ 特性**

1. **模組配置頁面新增“恢復預設佈局”按鈕**。
2. **新增 API 廣播事件** `app.lifeup.synthesis.complete`：當合成配方成功完成時會發送。
3. **世界模組搜尋支援按來源標籤篩選**：例如可在櫥窗頁面先篩選 API 型別商品，再進行搜尋。

**♻️ 最佳化**

1. **最佳化成就解鎖彈窗的字型展示效果**：在大字型情況下也能減少佈局異常。
2. **最佳化模組隱藏邏輯**：現可正確處理世界模組相關成就的顯示與隱藏。
3. **調整隨機任務建立後的預設行為**：不再自動領取到預設清單，避免使用者在未察覺的情況下被靜默領取。

**🐛 修復**

1. **修復任務列表重新整理時可能導致的罕見閃退問題**。
2. **修復 API 對外提供感想標題時的異常**：此前可能無法正確提供物品標題。
3. **修復番茄鍾橫屏介面的顯示問題**。
4. **修復 API 提供任務歷史記錄時，結束時間欄位的時間戳型別問題**。
5. **補充 API 返回欄位**：任務進度、結束狀態、重複結束條件等資訊現在可正確提供。
6. **修復任務詳情頁中，計數任務更新計數後數值未立即重新整理的問題**。
7. **修復一個可能導致番茄記錄時間異常的問題**。
8. **修復進入離線模式後首頁未立即重建的問題**：此前可能導致世界模組仍然顯示。
9. **修復隨機任務頁面可能卡在載入中的問題**。

**v1.102.2 - v1.102.7 (2026/02/03)**

**♻️ 最佳化**

1. **新增桌面小部件教程與文案最佳化**：新增相關引導，最佳化新增路徑與提示文案。

**🐛 修復**

1. **修復番茄鍾異常資料可能導致卡死**：異常資料場景下番茄鍾頁面不再進入死鎖/卡死狀態。
2. **修復追加計時息屏後時間不同步**：追加計時器在息屏或休眠後時間仍能正確同步。
3. **修復新建任務預設清單選擇邏輯**：在智慧清單內且預設清單被歸檔時，最佳化預設清單的選擇與回退邏輯。

**v1.102.1 (2026/01/27)**

**✨ 特性**

1. **圖片預覽支援縮放**：恢復了在頁面重構中丟失的圖片縮放功能，現在可以更方便地檢視圖片細節。

**🐛 修復**

1. **最佳化 WebDAV 下載記憶體佔用**：修復了在透過 WebDAV 下載時可能由於記憶體佔用過高導致的閃退或卡頓問題。
2. **修復感想圖片重新整理問題**：修復了在感想頁面編輯圖片後，圖片未能立刻同步重新整理的問題。

**v1.102.0 (2026/01/25, 滾動釋出途中被 v1.102.1 替代)**

**✨ 特性**

1. **音效管理上線**：支援匯入/試聽/刪除/複用音效，並可作為“物品使用效果”觸發播放。
2. **商店物品使用效果升級**：新增隨機經驗值、庫存變更、播放音效等效果，並最佳化配置流程。
3. **新增計時任務**：建立任務時可選擇“計時任務”，設定預期專注時長，達成目標後可完成並獲得獎勵。
4. **更靈活的周起始日**：支援選擇週一 / 週六 / 週日，日曆與統計等頁面同步生效。
5. **歡迎頁可跳過新手指引**：首次安裝可直接跳過新手引導，更快進入使用。
6. **圖示選擇支援直接輸入 Emoji**：在圖示選擇器中可輸入 Emoji（含組合 Emoji）快速生成圖示。
7. **日曆可檢視未來重複任務詳情**：從日曆點選未來日期的重複任務可正常進入詳情頁檢視資訊/統計等內容。
8. **API 能力增強**：URL Scheme API 新增/增強任務模板管理、重複結束條件、合成篩選跳轉等能力。

**♻️ 最佳化**

1. **效能與流暢度提升**：最佳化資料讀寫與索引策略，任務列表、倉庫歷史、統計等頁面在大資料量下更順滑。
2. **使用效果配置更順手**：最佳化使用效果選擇與展示互動，調整彈窗佈局與圖示表現。
3. **多語言體驗完善**：補全缺失的本地化文字。

**🐛 修復**

1. **修復倒計時效果編輯重置問題**：修復了編輯倒計時效果後確認會被重置為 1 的問題。
2. **修復團隊任務商品獎勵自動使用失效問題**：修復了團隊任務領取商品獎勵並勾選自動使用後，自動使用無法觸發的問題。
3. **修復開箱/簡易合成後不彈記錄感想問題**：修復了在開箱或簡易合成後，若同時存在“記錄感想”效果，結束後不會彈出記錄感想的問題。
4. **修復番茄鍾手動完成不彈結算彈窗**：修復了番茄鍾頁面手動完成任務時，獎勵/感想等結算彈窗未出現的問題，並恢復長按任務名稱完成任務的入口。
5. **修復小部件摺疊過濾失效問題**：修復了智慧清單小部件“摺疊未開始事項”過濾不生效的問題。
6. **修復部分場景閃退問題**：修復了在少數情況下因儲存狀態過大導致的閃退。
7. **修復日曆檢視未來任務詳情問題**：修復了從日曆檢視開啟未來重複任務詳情失敗的問題。
8. **修復記憶法重複任務異常**：修復了記憶法任務在少數情況下階段異常/重複生成的問題，並增加上限兜底。

**v1.101.8 (2026/01/12)**

**🐛 修復**

1. **修復重複結束條件設定問題**：修復了在“每 2 天”或自定義頻率下，無法設定事項重複結束條件的問題。
2. **修復番茄鍾計時異常變長問題**：修復了在未忽略電池最佳化時，因系統休眠或程序凍結導致計時時長異常增加的問題。

**v1.101.7 (2026/01/11)**

**🐛 修復**

1. **修復了一個可能會導致番茄計時時長異常縮短的問題**。

**v1.101.6 (2026/01/10)**

**🐛 修復**

1. **修復跟隨系統夜間模式切換的相關異常問題**。

**v1.101.5 (2026/01/08)**

**♻️ 最佳化**

1. **最佳化跟隨系統夜間模式切換邏輯**：修復了部分情況下 APP 無法隨系統自動切換深色/淺色模式的問題。
2. **最佳化獎勵兌換中的屬性選擇邏輯**：在「堅持天數獎勵」、「步數獎勵」及「獲贊獎勵」設定中，最佳化了未選擇屬性時的處理，現已支援取消選擇屬性，並修復了經驗值獎勵發放異常的問題。
3. **效能配置最佳化**：最佳化了內部配置項，以提升 APP 的執行效能。

**🐛 修復**

1. **修復統計查詢與圖表顯示異常**：修復了任務完成時間統計的相關查詢條件，解決了圖表資料展示不準確的問題。
2. **修復桌面小部件設定彈窗互動**：修復了小部件任務過濾設定彈窗中，缺少「確認」按鈕的問題。
3. **修復任務計數重新整理延遲**：修復了在任務詳情頁面更新計數後，首頁任務計數未同步重新整理的問題。
4. **修復感想 API 處理邏輯**：修復了感想 API 在處理「物品使用」型別時邏輯不正確的問題。
5. **修復日曆跳轉詳情異常**：修復了從日曆頁面跳轉至任務詳情時的相關問題。

**v1.101.4 (2025/12/30)**

**♻️ 最佳化**

1. **最佳化會員檢測更新邏輯**：提醒會員切換到“會員穩定渠道”更新，確保獲取最穩定的功能體驗。
2. **減少不必要的網路請求**：進一步節省使用者流量並降低伺服器成本。

**🐛 修復**

1. **修復成就進度更新問題（重點）**：修復了完成番茄記錄時，無法正常觸發成就進度更新的 Bug。

**v1.101.3 (2025/12/14)**

**🐛 修復**

1. 修復了「期限當天」選項可能忽略次日期限時間偏移的問題。

**v1.101.2 (2025/12/13)**

**🐛 修復**

1. 修復了商店、倉庫、任務頁面的搜尋狀態在返回頁面後被重置的問題。
2. 修復了部分情況下觸發系統 AlarmManager 500 個併發鬧鐘限制的問題。
3. 修復了動態取色、時區彈窗、選單彈窗等引發的若干崩潰問題。
4. 最佳化了崩潰上報邏輯，忽略了部分網路異常和格式異常。

**v1.101.1 (2025/12/01)**

**🐛 修復**

1. 修復了翻譯文字格式化可能導致的應用崩潰問題。

**v1.101.0 (2025/11/29)**

**✨特性**

1. **合成支援按商品篩選**：更快捷地定位可用配方與相關商品。
2. **商品詳情支援檢視合成配方**：在商品詳情頁直接檢視該商品的合成配方。
3. **商店頁面新增「合成」入口**：當商品可用於合成時，在購買按鈕旁展示「合成」按鈕。
4. **倉庫歷史篩選增強**：新增按日期、商品、描述等篩選。
5. **新增版本「新特性」顯示彈窗**：更新後首次進入展示本次版本亮點。
6. **番茄鍾重構並支援按任務設定專注時間**：為不同事項自定義專注時長。
7. **商品新增使用效果「使用後記錄感想」**：且感想頁面支援按商品篩選。
8. **番茄鍾生命週期 API 廣播事件**：新增生命週期相關廣播。
9. **簡單查詢 API**：支援查詢單個任務詳情資訊。
10. **計數任務自動遞增**：新增自動遞增能力。
11. **事項重複結束條件支援按日期結束**：新增按日期結束重複的互動與能力。
12. **小部件設定頁面改進**：最佳化應用內桌面小部件設定頁，可預覽支援的所有小部件，並在系統支援時快捷新增至桌面。

**♻️最佳化**

1. **清單管理頁面互動與夜間模式最佳化**：例如「全部」不再顯示“無法刪除”，改為禁用態按鈕。
2. **商店設定頁面獨立化**：調整為獨立頁面，並可從設定總頁面進入。
3. **事項預設背景彈窗文案調整**：說明更清晰。
4. **快速完成狀態標註**：開啟後，委託頁面頂部會標註快速完成開啟狀態。
5. **清單摺疊狀態按清單獨立記憶**：避免在「全部」摺疊後影響「日事項」清單。
6. **逾期彈窗夜間模式樣式最佳化**。
7. **感想頁面篩選按鈕顯示邏輯最佳化**：僅對支援篩選的感想型別生效（商品/事項）。
8. **團隊僅領取本地任務的互動最佳化**。
9. **合成頁面展示 UI 最佳化**：改進合成頁面的佈局與視覺反饋，提升呈現質量。
10. **商品相關互動最佳化**：增強商品互動流暢度，改善多場景使用感受。

**🐛修復**

1. 修復歷史頁面篩選後，頂部篩選欄未正確顯示的問題。
2. 修復某些情況下負面任務未按正確的 1× 懲罰次數計算懲罰的問題。
3. 修復番茄鍾選擇事項時，在某些情況下“取消選擇”選項會異常消失的問題。
4. 修復跟隨系統夜間模式時的若干異常展示與切換問題。
5. 修復小部件完成計數任務後未彈出獎勵提示的問題。

**v1.100.6 (2025/11/08)**

**🐛修復**

1. 修復選擇物品時，輸入框可能因外接輸入裝置（鍵盤/手柄等）導致聚焦溢位從而崩潰的問題（修復效果待確認）。
2. 修復智慧時區管理彈窗無法滾動檢視底部內容和按鈕的問題。

**v1.100.5 (2025/09/28)**

**✨特性**

1. **自定義音效支援內建音效選擇**：新增內建音效庫選擇功能，提供更便捷的個性化音效體驗。
2. **合成篩選功能增強**：合成頁面新增「僅顯示可合成」篩選選項，提升物品管理效率。
3. **API 表情符號支援**：商品、屬性、成就等 API 現在支援直接傳入 emoji 表情符號，增強個性化定製。
4. **團隊建立功能改進**：建立團隊時支援選擇領取團隊任務的目標清單，更好地組織管理任務。
5. **團隊任務複製功能**：支援將團隊任務複製為本地任務，無需加入團隊即可使用。
6. **任務 API 功能增強**：任務相關 API 新增引數支援設定「淺色備註字型」狀態。

**♻️最佳化**

1. **"是否放棄更改"邏輯最佳化**：改進編輯商品、合成、成就、成就清單等頁面的確認彈窗邏輯，僅在實際修改內容後才提示。
2. **成就解鎖條件顯示最佳化**：成就解鎖條件現在預設展開顯示，提升可見性和使用者體驗。
3. **合成頁面效能最佳化**：最佳化合成詳情頁面的查詢效能，提升響應速度。
4. **Toast API 穩定性提升**：改進 toast API 的穩定性和可靠性，減少異常情況。
5. **團隊任務完成流程最佳化**：增強團隊任務完成流程，提供更好的錯誤處理和使用者引導。
6. **合成拖拽行為改進**：最佳化合成頁面自定義排序時拖動到邊緣的列表滾動行為。
7. **團隊任務領取流程最佳化**：改進領取團隊任務後的工作流程，支援跳轉到對應清單。
8. **日曆提醒邏輯最佳化**：改進日曆提醒相關邏輯，提升功能可靠性。

**🐛修復**

1. 修復老版本自定義背景錯誤使用全域性預設背景「淺色備註字型」狀態的問題。
2. 修復自定義音效頁面狀態列適配問題。
3. 修復成就描述可能與解鎖按鈕重疊的顯示問題。
4. 修復合成詳情頁面拖拽排序時無法滾動的問題。
5. 修復世界模組放置到側邊欄後，搜尋按鈕可能偶現消失的問題。
6. 嘗試修復夜間模式跟隨系統的異常問題。
7. 嘗試修復連續完成團隊任務時可能導致任務重複的問題。
8. 修復解鎖成就時「感想」功能失效的問題。

**v1.100.4 (2025/09/07)**

**♻️最佳化**

1. **崩潰報告改進**：最佳化崩潰採集和報告機制，便於更好地分析和除錯問題。

**🐛修復**

1. 修復由 Facebook SDK 引起的崩潰問題。
...
1. 修復商店新建或編輯商品時，某種情況下**沒法選擇預設清單**的問題。

**v1.100.1 (2025/09/03)**

**✨特性**

1. **自定義背景支援備註字型顏色選項**：進一步個性化介面體驗。
2. **物品使用效果增強**：針對隨機金幣減少效果也支援啟用「限制使用」功能。

**♻️最佳化**

1. **日曆提醒功能最佳化**：增加選項可以調整插入的日曆提醒事件的持續時長。
2. **任務編輯互動最佳化**：編輯任務時的放棄更改彈窗顯示邏輯得到改進，當沒有編輯內容的情況下退出不再展示。
3. **多語言本地化更新**：更新本地化多語言文字，提升使用者體驗。

**🐛修復**

1. 修復**相容性配置頁面和提醒方式設定頁面**不適配夜間模式的問題。
2. 嘗試修復**多個頁面的彈窗與輸入法焦點相關的崩潰閃退**問題，提升應用穩定性。
3. 修復**編輯成就沒法更改是否自動使用物品**的問題。

**v1.100.0 (2025/07/29)**

**✨特性**

1. **番茄計時、經驗值、倉庫歷史、金幣詳情** 增加一鍵跳轉對應統計頁面的功能。
2. 支援更靈活的提醒事件設定（開始前、期限前 X 分鐘）。
3. 支援隱藏合成清單。
4. 支援更換步數、堅持獎勵的屬性。
5. 支援自定義側邊欄功能模組（如將商店、社羣放到側邊欄或隱藏不需要的模組）。
6. 增加實驗選項 **「低限制模式」**：放寬 app 內的數值限制（如經驗值、金幣位數、任務可選擇的屬性數量）。
7. 最佳化逾期處理彈窗的 UI 和互動邏輯。
8. 增加自動使用商品選項的說明。
9. 更多重複解鎖條件支援：
   - 每日內獲得番茄數。
   - 每日內番茄專注時長。
   ...
   - 每日使用 N 次某物品。
   - 每日完成某個指定任務達到 N 次。
10. 智慧清單選項遷移到清單管理彈窗（任務列表頁面頂部點選清單按鈕）。
11. 增加 **「快速完成」** 選項：啟用後完成任務將跳過所有彈窗。
12. 擴充套件自定義金幣圖示的影響範圍，現支援單色圖示（如商店頂部的金幣圖示）。
13. 商品詳情現在會標識商品所屬清單，方便從倉庫確認商品歸屬。
14. 編輯合成配方時，支援拖拽排序和點選編輯商品。
15. 增加直接編輯金幣數的 API。
16. 查詢 API 支援查詢番茄資訊（番茄數）。
17. 部分 API 的數值限制放寬（預設 API 為低限制）。
18. **重新最佳化和設計的成就條件進度計算機制**：提升計算效能和進度更新速度。

**♻️最佳化**

1. 最佳化 **「我的」** 頁面的堅持天數，支援手動點選重新計算。
2. 修復日曆頁面的 RTL 佈局問題，周起始日設定為 **「週一」**（此前為週日）。
3. 任務、商店智慧清單內的分組顯示支援按組標籤摺疊/展開。
4. 任務列表在底部展開已完成、未開始、凍結時，頂部也會出現對應標籤。
5. 世界模組匯入的商品圖示持久化儲存在本地，避免無網路時無法載入。
6. 清單列表彈窗現在會標識智慧清單。
7. 最佳化團隊任務、隨機任務的預設清單邏輯：當預設清單被歸檔時，預設選擇第一個清單。
8. 最佳化計數任務的逾期處理邏輯：如果已達成計數，預設設定為 **「已完成」**。

**🐛修復**

1. 修復編輯成就時，無法正確恢復 **「自動使用商品」** 勾選狀態的問題。
2. 修復智慧清單的頂部卡片計數問題：未排除屬於歸檔清單的任務。
3. 修復商品購買的 **「自動使用」** 勾選狀態全域性記憶問題，改為按商品獨立記憶。
4. 修復解鎖成就 API 在某些情況下無法正確更新進度的問題。
5. 修復團隊任務、隨機任務的預設清單邏輯問題。
6. 技術依賴庫升級，目標 API 版本調整至 35（Android 15）。

#### Alpha/Beta 補丁修復

1. 移除不必要的依賴，適配 16K page size，**減少應用包體積**。
2. 修復 **自動使用物品** 功能邏輯：對於 URL 商品，現在只會使用 1 件，其餘會放到倉庫。（此前也只會生效 1 件，但不會放到倉庫，會損失使用效果）
3. 修復 **重複成就條件** 無法重新計算進度的問題。
4. 修復 **備份檔案** 不包含任務模板的問題。
5. 修復恢復備份後，大部分 emoji 會重新生成異常圖示的問題。
6. 更新應用內反饋的 **QQ 頻道連結**。
7. 增加 **感想釋出廣播** 功能。
8. **重新設計** 相容性配置和提醒方式設定介面。
9. 為 **減少金幣效果** 增加「限制物品使用」可選項。
10. **API 相關更新**：任務 API 支援激勵語引數。
11. 修復新事項預設設定頁面的頂部間距異常問題。

**v1.99.5 (2025/07/29)**

**🐛修復**

1. 嘗試修復某些情況下，偶現自定義背景沒有正確顯示的問題。

**v1.99.3 (2025/06/30)**

**✨特性**

1. 子任務 API 支援相對調整（set_type）。
2. 支援自動清理過期的日曆提醒事件。
3. 最佳化歸檔清單的處理邏輯：

- 智慧清單不再展示已歸檔任務。

- 歸檔清單中的任務預設不執行自動推進邏輯（與凍結狀態類似）。

**♻️最佳化**

1. 連續新增任務/成就時，支援自動滾動到頂部並聚焦輸入框。
2. 最佳化完成負面任務時的相關文案措辭。
3. 最佳化編輯任務頁面中“放棄變更”彈窗的展示邏輯。
4. 最佳化日曆提醒的事件持續時長，以避免在某些裝置上可能出現的問題。

**🐛修復**

1. 修復小部件不支援展示自定義金幣圖示的問題。
2. 修復任務詳情頁面不支援展示多物品獎勵的問題。
3. 修復某些場景（如小部件）可能不遵循清單排序規則的問題。

**v1.99.1-rc02 (2025/06/20)**

**✨特性**

1. 支援「每連續完成 N 次任務」的成就重複解鎖條件
2. 新建/編輯任務 API 新增任務型別、金幣/經驗值相對調整功能
3. 成就 API 支援設定金幣，以及支援金幣/經驗值相對調整功能
4. API 支援跳轉到具體成就清單和合成清單

**♻️最佳化**

1. 最佳化開箱時產生的倉庫物品歷史記錄順序
2. 統計頁面的篩選項支援記憶
3. 篩選頁面的篩選項支援全選操作
4. 加強了任務的建立去重攔截邏輯
5. 任務詳情頁面的操作補充：凍結、調整期限
6. 支援顯示合成清單 id

**🐛修復**

1. 修復此前任務 API 無法建立/編輯艾賓浩斯任務的問題
2. 修復從任務列表小部件啟動 app 時，任務清單顯示和頂欄的清單名稱顯示可能不一致的問題
3. 修復簡潔模式卡片的文字可能顯示不全的問題
...
1. 新增月、年重複事項的重複錨定日設定功能：支援直接設定錨定的月日期（如月末）。
2. 最佳化多選商品的相關互動：預設進入多選模式，不再區分單選/多選，重新選擇時可恢復之前的選項。
...
1. 修復商品的商店庫存不足時，仍可透過 API 購買的問題。
2. 修復任務模板無法恢復自動金幣獎勵的問題。
3. 修復任務標題處長按偶爾失效的問題。
4. 修復某些特殊情況下，新建商品可選中已刪除清單的問題。
5. 修復編輯任務時，提醒時間處未正確顯示清除按鈕的問題。
...
1. **唯讀 HTTP 介面支援 ETag**：回應會帶上 `ETag`。請求帶 `If-None-Match` 且內容未變化時回傳 304，不必重複下載。官方 MCP 也會自動沿用未變化的讀取結果。

**v3.0.1（2026/09/13）**

**✨ 特性**

1. **HTTP/MCP 讀取介面補齊商品與成就欄位**：商品可讀取標題顏色、操作文案、下架/禁用使用狀態、購買/使用限制及限制範圍；成就可讀取顏色。需配合 LifeUp **1.106.0+**。

**♻️ 最佳化**

1. **WebSocket 事件推送預設開啟**（可在進階設定關閉）；HTTP `GET /events` 拉取仍可用。

**v3.0.0（2026/08/28）**

**ℹ️ 重要說明**

1. **需配合 LifeUp 1.106.0+**：流水帳、統計資料、等級曲線、廣播事件等新 API 依賴新版人升 App。

**✨ 特性**

1. **MCP 與 Skills 橋接**：支援 Cursor、Claude 等 AI 用戶端透過區域網路連線 Cloud，讀取並操作人升資料。詳見 [MCP 文件](https://wiki.lifeupapp.fun/zh-hant/index.html#/guide/api_mcp)。
2. **掃描 lifeup:// API QR 碼**：掃碼按鈕旁新增使用說明，便於自動化工具快速呼叫。
3. **lifeupcloud:// 快捷控制**：狀態卡片提供 `start` / `stop` 快捷連結，便於 Tasker 等工具啟停 Cloud 服務。
4. **廣播事件介面**：新增 HTTP/WS `/events`，可接收人升 App 廣播（需在 LifeUp **設定 → 實驗室 → 開發者模式 → 廣播事件** 開啟；Cloud 進階設定可查看狀態並一鍵跳轉開啟）。
5. **大幅擴充 HTTP 讀取 API**：金幣/背包/經驗流水、步數記錄、統計資料、等級曲線、技能分組、成就解鎖條件等；分類列表支援 `include_hidden`。

**♻️ 最佳化**

1. **區域網路探索改進**：mDNS 公告真實 HTTP 連接埠，連線更可靠。
2. **支援 Android 16**；App 內已有 14 種語言。
...
1. 在 SDK 資料模型與 HTTP API 響應中暴露子任務完成時間（`endTime`）欄位。

**v2.1.0（2026/03/22）**

**🚀功能**

1. 新增語言配置支援，並補齊 Top 14 本地化。
2. 暴露任務計數進度與重複結束條件介面欄位。

**🐛 修復**

1. 修復了 ContentProvider 相容性問題。
2. 適配 Android 15 的介面與服務啟動行為。
