...
| 引數   | 含義       | 取值                                         | 示例 | 是否必須                | 備註                                                         |
| ------ | ---------- | -------------------------------------------- | ---- | ----------------------- | ------------------------------------------------------------ |
| key    | 查詢的型別 | 僅限以下數值其一：<br/>coin<br/>atm<br/>item | coin | 是                      | coin - 當前金幣數<br/>atm - 當前 ATM 存款<br/>item - 指定 `itemId` 的商品資訊 |
| itemId | 商品id     | 大於 0 的數字                                | 1    | 當 key 為 item 時，必須 | 查詢的商品 id                                                |
