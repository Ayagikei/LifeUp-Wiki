...
> [!NOTE]
> 我們正在釋出《人升》v1.106.0 以及《雲人升》v3.0.0。由於各平臺稽覈時間不一，文件可能已提前更新，App 實際上線時間可能會有所延誤。
...
本文的 API 引數和定義基於 v1.106.0 版本編寫。

使用 API 前，建議將應用升級到 v1.106.0 版本，如果沒法檢測到更新，請切換更新渠道到【會員內測-嚐鮮版】。
...
當介面支援透過名稱查詢任務、商品或子任務時，會先嚐試忽略大小寫的完整名稱匹配；如果沒有完整匹配，纔會回退到包含關係的模糊匹配。

例如同時存在「金幣」和「金幣箱」時，傳入 `name=金幣` 會優先匹配「金幣」，而不是較早出現在搜索結果中的「金幣箱」。

如果同名或近似名稱可能造成歧義，仍建議優先傳入對應的 id。
...
`purchase_limit` 是一個 JSON 陣列，其中每個物件代表一條限制規則。

| 欄位 | 含義 | 型別 | 是否必須 | 備註 |
| ---- | ---- | ---- | -------- | ---- |
| limitType | 限制類型 | 數字 | 是 | 見下方型別表 |
| limitNumber | 主要數值 | 數字 | 否* | 用於次數/區間類規則 |
| maxNumber | 區間上限 | 數字 | 否 | 用於屬性等級區間、擁有物品數量區間 |
| limitId | 關聯目標 ID | 數字 | 否* | 屬性 / 物品 / 任務 / 任務週期 / 成就類規則需要 |
| extendInfo | 擴充套件資訊 | 字串 | 否 | 用於時間類規則；欄位值本身是一個 JSON 字串 |

**型別定義**

| limitType | 含義 | 必填欄位 / 說明 |
| --------- | ---- | --------------- |
| 0 | 每日週期數量限制 | `limitNumber`：每天最多次數 |
| 1 | 每週週期數量限制 | `limitNumber`：每週最多次數 |
| 2 | 每月週期數量限制 | `limitNumber`：每月最多次數 |
| 3 | 每年週期數量限制 | `limitNumber`：每年最多次數 |
| 10 | 屬性等級限制 | `limitId`：屬性 ID<br/>`limitNumber`：最低等級<br/>`maxNumber`：可選，最高等級 |
| 20 | 每日時間段限制 | `extendInfo`：`{"startMinuteOfDay":540,"endMinuteOfDay":1320}` |
| 21 | 每週幾限制 | `extendInfo`：`{"weekdays":[1,2,3,4,5]}`<br/>週一 = 1，週日 = 7 |
| 22 | 絕對時間段限制 | `extendInfo`：`{"startMillis":1710000000000,"endMillis":1710086400000}` |
| 23 | 月份限制 | `extendInfo`：`{"months":[1,6,12]}` |
| 24 | 每月日期限制 | `extendInfo`：`{"daysOfMonth":[1,15,31]}` |
| 30 | 擁有某物品數量限制 | `limitId`：目標物品 ID<br/>`limitNumber`：最小擁有數量<br/>`maxNumber`：可選，最大擁有數量 |
| 31 | 完成某任務限制 | `limitId`：任務 ID |
| 32 | 解鎖某成就限制 | `limitId`：成就 ID |
| 33 | 按任務週期完成限制 | `limitId`：重複任務組 ID（`groupId`）<br/>按該任務組最新已開始週期是否完成來判定 |

**說明**

- 為相容舊寫法，可以不傳 `maxNumber` 與 `extendInfo`。
- `limitType=33` 時，`limitId` 不是任務 ID，而是重複任務的 `groupId`。
- `extendInfo` 本身是字串欄位，因此透過 URL 呼叫 API 時，內部 JSON 字串通常還需要額外轉義/編碼一次。
...
| number    | 獎勵數量         | 大於 0 的數字                          | 1           | 是       | 如果是金幣，取值最大為999999<br/>如果是經驗值，取值最大為99999<br/>如果是商品，取值最大為999999（超出按上限發放）<br/>開啟「設定 → 實驗室 → 減少應用限制」後，金幣/商品上限為999999999，經驗值上限為99999999 |
...
| weekdays        | 每週幾         | `1,3,5` 或 `none` | 1,3,5     | 否       | v1.106.0+；1=週一 … 7=週日，列出**要重複**的星期。出現時 frequency 須省略或為 1。add 不接受 `none`；edit 傳 `none` 清回每日。七天都給時按每日處理 |
| importance      | 重要程度       | [1, 4]            | 1         | 否       | 預設為 1                       |
| difficulty      | 困難程度       | [1, 4]            | 1         | 否       | 預設為 1                       |
| deadline        | 截止時間       | 時間戳（毫秒）     | 1640995200000 | 否    |                               |
| no_deadline     | 無期限         | true/false         | true      | 否       | v1.104.0+；僅對重複任務有效，傳入 `&no_deadline=true` 時會清除具體截止時間 |
...
| enable_outline  | 啟用文字輪廓   | true 或者 false    | false      | 否       | 僅當設定 background_url 時有效，為文字新增輪廓以提高可讀性 |
| use_light_remark_text_color | 使用淺色備註文字 | true 或者 false | false | 否 | 僅當設定 background_url 時有效，使用淺色顯示備註文字 |
...
| enable_proportional_settlement | 啟用按比例結算 | true/false | false | 否 | v1.104.0+；僅當 task_type 為1(計數任務)時有效。開啟後，計數進度結算會按完成比例發放獎勵，最終完成不會重複發放已結算獎勵 |
...
| id   | 任務id   | 大於 0 的數字 | 1    | 否*      | 任務id；如果是重複任務，每次重複，id都會更新。 |
| gid  | 任務組id | 大於 0 的數字 | 1    | 否*      | 任務組id；                                     |
| name | 名稱     | 任意文字      | 早起 | 否*      | 模糊搜尋，僅會對搜索到的其中一個任務執行放棄操作 |
...
| id   | 任務id      | 大於 0 的數字 | 1             | 否*      | 任務id；如果是重複任務，每次重複，id都會更新。 |
| gid  | 任務組id    | 大於 0 的數字 | 1             | 否*      | 任務組id；                                     |
| name | 名稱        | 任意文字      | 早起          | 否*      | 模糊搜尋，僅會對搜索到的其中一個任務執行凍結操作 |
...
| id   | 任務id   | 大於 0 的數字 | 1    | 否*      | 任務id；如果是重複任務，每次重複，id都會更新。 |
| gid  | 任務組id | 大於 0 的數字 | 1    | 否*      | 任務組id；                                     |
| name | 名稱     | 任意文字      | 早起 | 否*      | 模糊搜尋，僅會對搜索到的其中一個任務執行解凍操作 |
...
| id   | 任務id   | 大於 0 的數字 | 1    | 否*      | 任務id；如果是重複任務，每次重複，id都會更新。 |
| gid  | 任務組id | 大於 0 的數字 | 1    | 否*      | 任務組id；                                     |
| name | 名稱     | 任意文字      | 早起 | 否*      | 模糊搜尋，僅會對搜索到的其中一個任務執行刪除操作 |
...
| frequency       | 重複頻次       | 整數               | 0         | 否       | 預設為 0（單次）<br/>0 - 單次<br/>1 - 每日<br/>N (N>1) - 每 N 日<br/>-1 - 無限<br/>-3 - 艾賓浩斯（需v1.99.1）<br/>-4 - 每月<br/>-5 - 每年 |
| weekdays        | 每週幾         | `1,3,5` 或 `none` | 1,3,5     | 否       | v1.106.0+；與 add_task 相同。省略則不改；`none` 清回每日；改 frequency 為非 1 時會清掉每週幾 |
| importance         | 重要程度       | [1, 4]            | 1        | 否       | 預設為 1                       |
...
| target_times    | 目標次數       | 大於0的整數        | 1         | 否       | 僅當 task_type 為1(計數任務)時有效 |
| is_affect_shop_reward | 是否影響商店獎勵 | true/false      | false    | 否       | 當 task_type 為1(計數任務)時有效，是否影響商品的獎勵計算         |
| enable_proportional_settlement | 啟用按比例結算 | true/false | false | 否 | v1.104.0+；僅當 task_type 為1(計數任務)時有效。編輯現有計數任務時，可單獨傳入該引數來開關按比例結算；改變該設定或獎勵配置時，可能按應用內規則重置/回滾已結算進度 |
| expected_focus_minutes | 預期專注分鐘數 | 大於 0 的整數 | 25 | 否 | 僅當 task_type 為4(計時任務)時有效；預設 25（v1.102.0+） |
...
| weekdays | 每週幾 | `1,3,5` | 1,3,5 | 否 | v1.106.0+；與 add_task 相同，create 不接受 `none` |
...
| enable_proportional_settlement | 啟用按比例結算 | true/false | false | 否 | v1.104.0+；僅當 task_type 為 1(計數任務)時有效。模板生成任務後會保留該計數任務結算設定 |
...
| purchase_limit  | 限制規則       | JSON文字          | 參見[購買限制結構](#3-購買限制結構) | 否 | 可配置購買/使用限制 |
...
| purchase_limit   | 限制規則       | JSON文字           | 參見[購買限制結構](#3-購買限制結構) | 否 | 傳 `null` 可清空全部限制 |
| limit_scope      | 限制作用範圍   | purchase / use / both | purchase | 否 | 僅在傳入該欄位時纔會更新；當 `purchase_limit` 被清空時會自動一併清空 |
...
1. 舊版 `loot_box` API 保持相容語義：如果同一內容物存在多個不同數量的條目，會編輯匹配到的第一條，不能用 `sub_amount` 消歧。需要按數量精確編輯、刪除或合併條目時，請使用 `loot_box/v2`。

<br/>

#### 調整開箱效果（v2）

?> 該 API 於 v1.104.2 版本引入。

**方法名：**loot_box/v2

**說明：**loot_box API 的改進版本，修改指定箱子的開箱效果，支援調整機率、獎勵數、增加內容物和**刪除內容物**。

**相比 v1 的改進：**
- **`sub_amount` 精準匹配**：當開箱中存在同一物品的多個不同數量條目時（如 A x1 50%、A x2 30%），用 `sub_amount` 精準定位指定條目。預設值為 `1`。匹配不到時按 `sub_id` / `sub_name` 查詢商品並新增條目；如果本次請求是 `amount=0` 刪除語義，則不會新增。
- **`set_type` 獨立控制**：`amount_set_type` 和 `probability_set_type` 可獨立設定，全域性 `set_type` 作為預設回退值。
- **支援刪除條目**：`amount=0`（absolute 模式）或計算後 `<=0`（relative 模式）時刪除匹配到的條目。
- **重複條目合併**：如果調整 `amount` 後與同一箱子中已有的相同物品、相同數量條目重複，會合併到已有條目，並繼續應用本次傳入的 `probability` / `fixed`。

**示例：**[lifeup://api/loot_box/v2?name=金幣箱&sub_name=【大】袋金幣&sub_amount=2&probability_set_type=relative&probability=10](lifeup://api/loot_box/v2?name=金幣箱&sub_name=【大】袋金幣&sub_amount=2&probability_set_type=relative&probability=10)

**解釋：**增加金幣箱中的【大】袋金幣（x2）的比重 10 點。

| 引數                 | 含義                  | 取值                                     | 示例         | 是否必須 | 備註                                                         |
| -------------------- | --------------------- | ---------------------------------------- | ------------ | -------- | ------------------------------------------------------------ |
| id                   | 商品id                | 大於0的數字                              | 1            | 否*      | 獲取方式請檢視上文 「基礎知識 - 人升資料 ID」                |
| name                 | 商品名稱              | 任意文字                                 | 金幣箱       | 否*      | 用於未知 id 時，模糊搜尋商品，並非修改名稱                   |
| sub_id               | 箱子內容物的 id       | 大於0的數字                              | 1            | 否*      | 箱子內容物的 id。與 sub_name 同時提供時，優先使用 sub_id 匹配 |
| sub_name             | 箱子內容物的名稱      | 任意文字                                 | 【大】袋金幣 | 否*      | 用於箱子內容物未知 id 時，模糊搜尋商品                       |
| sub_amount           | 箱子內容物數量（匹配） | 數字                                    | 2            | 否       | 精準匹配 amount 等於該值的條目，最小值為 `1`，預設 `1`。匹配不到且不是刪除語義時新增條目。 |
| set_type             | 全域性調整方式          | `absolute` / `relative`                  | relative     | 否       | 作為 amount_set_type 和 probability_set_type 的預設值        |
| amount_set_type      | 獎勵數調整方式        | `absolute` / `relative`                  | relative     | 否       | 覆蓋 set_type 對 amount 欄位的設定方式                       |
| probability_set_type | 獎勵比重調整方式      | `absolute` / `relative`                  | absolute     | 否       | 覆蓋 set_type 對 probability 欄位的設定方式                  |
...
1. 刪除條目：設定 `amount=0`（absolute 模式）或使用 relative 模式使計算結果 `<=0`。刪除僅對匹配到的已有條目生效；如果沒有匹配到已有條目，`amount=0` 不會新增內容物。
1. 如果把某個內容物的 `amount` 調整為同一箱子內已有的相同物品、相同數量條目，會合併到已有條目，並用本次請求繼續覆蓋 `probability` / `fixed`。
1. 當刪除後開箱為空時，整個開箱效果會被軟刪除（商品本身保留，後續仍可重新新增開箱條目）。
...
| 引數   | 含義     | 取值 | 示例             | 是否必須 | 備註                                                         |
| ------ | -------- | ---- | ---------------- | -------- | ------------------------------------------------------------ |
| result | 結果碼   | 數字 | 0                | 是       | 0 - 使用成功<br/>1- 資料庫異常<br/>2 - 經驗值不足限制<br/>3 - 找不到商品<br/>4 - 執行倒計時衝突<br/>5 - 庫存不足<br/>6 - 商品不可使用<br/>7 - 金幣限制<br/>8 - 目標庫存限制<br/>9 - 屬性等級限制<br/>10 - 時間限制<br/>11 - 擁有物品數量限制<br/>12 - 任務完成限制<br/>13 - 成就解鎖限制<br/>14 - 週期數量限制<br/>15 - 任務週期完成限制 |
...
| task_id         | 任務id                 | 大於 0 的數字                 | 1             | 否       | 獲取方式請檢視上文 「基礎知識 - 人升資料 ID」 |
| task_gid        | 任務組id               | 大於 0 的數字                 | 1             | 否       | 獲取方式請檢視上文 「基礎知識 - 人升資料 ID」 |
| task_name       | 名稱                   | 任意文字                      | 學習          | 否       | 模糊搜尋，只會搜索到的其中一個任務            |
...
| image_uris               | 圖片 URI   | URI 字串列表                 |                | 否       | 支援本地檔案 URI (file://) 或 遠端網路圖片 (http/https)。支援陣列（如 &image_uris=uri1&image_uris=uri2） |
| image_uris_update_mode | 更新模式   | APPEND 或者 REPLACE | REPLACE | 否       | 僅當更新現有記錄且提供 image_uris 時有效<br/>APPEND：追加圖片<br/>REPLACE：替換現有圖片（預設）        |
...
如果商品配置了 `purchase_limit`，且 `limit_scope` 包含 `purchase`，該介面也會校驗這些限制。
...
| 6    | NotPurchaseable           | 因商品設定或限制條件而不可購買 |
| 7    | OutOfStock                | 商店庫存不足                   |
...
- 該介面對應的是**配方合成**。
- 當開啟`廣播事件`且配方合成成功後，人升還會額外發送 `app.lifeup.synthesis.complete` 廣播事件。
- 該廣播**不會**用於 `use_item` 中的簡易合成；簡易合成仍然歸屬於 `app.lifeup.item.use`。
...
| icon_uri     | 圖示           | emoji、http(s)、content URI 或空 | 🏆 | 否 | 不是 `icon`。可直接傳 emoji。空字串清除圖示。 |
...
| coin         | 金幣獎勵       | [0, 999999]      | 10         | 否       | 解鎖成就時獲得的金幣數量        |
| coin_var     | 金幣獎勵浮動值 | 整數              | 5          | 否       | 金幣獎勵的浮動範圍             |
| coin_set_type| 金幣獎勵型別   | 以下數值其一：<br/>absolute<br/>relative | absolute | 否 | absolute - 直接設定金幣為 value<br/>relative - 在原金幣值的基礎上增加或減少 |
| exp_set_type | 經驗值獎勵型別 | 以下數值其一：<br/>absolute<br/>relative | absolute | 否 | absolute - 直接設定經驗值為 value<br/>relative - 在原經驗值的基礎上增加或減少 |
...
| 19      | ATM存款           | 否              | -              | 存款數量           |
| 20      | 外部API           | 否              | -              | 根據API定義        |
| 520     | 每日完成 N 個不同任務 | 否            | -              | 不同任務數（按 groupId 去重；已有型別） |
| 524     | 每日累計完成 N 次任務 | 否            | -              | 當日有效完成記錄總次數（v1.104.4+） |

> 自 v1.104.4 起，型別 `520` 與 `524` 的語義如下：
>
> - 兩者共享相同的完成口徑與本地自然日邊界（`TimeRange.today()`）。
> - 普通任務統計 `COMPLETED`，負面任務統計 `GIVE_UP`。
> - 型別 `520` 按有效 `groupId` 去重（缺失 groupId 時回退到任務記錄 id）。同一無限任務當天多次完成只計 1 個不同任務。
> - 型別 `524` 按有效完成記錄逐行計數。同一無限任務當天完成 5 次則 `completionCount = 5`。
> - 已有 `type=520` 的成就保持不同任務語義，無需遷移。
...
[
   ...
   {
       "type": 520,         // 每日完成 N 個不同任務
       "related_id": null,
       "target": 5
   },
   {
       "type": 524,         // 每日累計完成 N 次任務
       "related_id": null,
       "target": 10
   }
]
...
| type        | 型別           | 整數               | 0          | 否       |                               |
| order       | 排序           | 整數               | 1          | 否       | 原始混排位置。單獨傳時按該位置落位，並由位置推導最終分組；與 `group_id` 一起傳時，會在目標組內吸附到最接近的合法位置 |
| group_id    | 技能組ID       | 整數               | 10         | 否       | 需要 v1.103.0+；不能小於 `0`，僅 `0` 表示移到未分組區域。單獨傳時：新建技能會追加到目標組尾部，編輯技能若已在該組則儘量保留當前位置，否則移動到組尾；與 `order` 一起傳時以 `group_id` 為主 |
| status      | 狀態           | 整數               | 0          | 否       |                               |
...
### 技能組管理 :id=skill_group_management

需要 v1.103.0+

**方法名：**skill_group

**說明：**建立、編輯、刪除技能組，也支援一次性提交技能組與技能的混排排序結果。

**示例：**

- 建立技能組：[lifeup://api/skill_group?content=戰鬥](lifeup://api/skill_group?content=戰鬥)
- 編輯技能組：[lifeup://api/skill_group?id=10&content=戰鬥&order=20&collapsed=true](lifeup://api/skill_group?id=10&content=戰鬥&order=20&collapsed=true)
- 刪除技能組：[lifeup://api/skill_group?id=10&delete=true](lifeup://api/skill_group?id=10&delete=true)
- 混排排序：
...
| 引數 | 含義 | 取值 | 示例 | 是否必須 | 備註 |
| ---- | ---- | ---- | ---- | -------- | ---- |
| id | 技能組ID | 大於 0 的數字 | 10 | 否* | 編輯或刪除時必須提供 |
| content | 技能組名稱 | 任意文字 | 戰鬥 | 否* | 新建時必須提供 |
| order | 排序值 | 整數 | 20 | 否 | 原始 `orderInCategory`；在技能與技能組的混排列表中必須唯一 |
| collapsed | 摺疊狀態 | true 或 false | true | 否 | 是否摺疊該技能組 |
| delete | 是否刪除 | true 或 false | false | 否 | 僅編輯模式下有效 |
| sort_json | 混排排序節點 | JSON 陣列 | `[{"type":"skill","id":2},{"type":"group","id":10}]` | 否* | 提供後會忽略 CRUD 引數，直接按該排序計劃回寫。支援部分排序：未傳入的節點會保持原相對順序 |

`sort_json` 節點格式：

| 欄位 | 含義 | 取值 |
| ---- | ---- | ---- |
| type | 節點型別 | `skill` / `group` |
| id | 實體 ID | 大於 0 的數字 |
...
| 欄位名 | 型別 | 說明 | 示例 | 備註 |
| ------ | ---- | ---- | ---- | ---- |
| id | 數字 | 技能組ID | 10 | 建立 / 編輯 / 刪除時返回 |
| count | 數字 | 參與排序的節點數量 | 3 | 使用 `sort_json` 時返回 |
...
| task_gid / taskGid / task_group_id / taskGroupId | 任務組 ID | 大於 0 的數字 | 1 | 當 key 為 task 時，三選一* | 查詢的任務組 ID |
| task_name / taskName | 任務名稱 | 任意文字 | 學習 | 當 key 為 task 時，三選一* | 模糊匹配任務名稱 |
| withSubTasks | 是否包含子任務 | true 或 false | true | 否 | 僅 key 為 task 時可用，預設為 true |
...
| desc             | 描述         | 任意文字 |           | 否       | -                              |
| icon             | 圖示         | 任意文字 |           | 否       | -                              |
| category_id      | 類別id       | 數字     |           | 否       | -                              |
| stock_number     | 庫存數量     | 數字     | -         | 是       | -1代表無限庫存                 |
| own_number       | 倉庫擁有數   | 數字     | 10        | 是       | -                              |
| price            | 價格         | 數字     | 100       | 是       | -                              |
| order            | 排序依據     | 數字     | 100       | 是       | 自定義排序時的權重值           |
| disable_purchase | 是否禁止購買 | true 或 false | true | 是 | -                              |
| purchase_limit   | 限制規則     | JSON文字 | [{"limitType":0,"limitNumber":5}] | 是 | 當前商品的限制列表 |
| limit_scope      | 限制作用範圍 | purchase / use / both | use | 是 | 以 API 文字值返回 |
...
| name           | 任務名稱         | 文字      | 學習 | 是       | -                              |
| notes          | 備註             | 文字      | -    | 否       | 可能為空                       |
| status         | 任務狀態         | 數字      | 0    | 是       | 0=未完成, 1=已完成             |
| startTime      | 開始時間         | 數字      | -    | 是       | Unix時間戳(毫秒)               |
| deadline       | 截止時間         | 數字      | -    | 否       | Unix時間戳(毫秒),可能為空      |
| remindTime     | 提醒時間         | 數字      | -    | 否       | Unix時間戳(毫秒),可能為空      |
| frequency      | 重複頻率         | 數字      | -    | 是       | -                              |
| weekdays       | 每週幾           | 文字      | 1,3,5 | 否      | v1.106.0+；非每週幾為空字串。1=週一 … 7=週日 |
| exp            | 經驗值獎勵       | 數字      | -    | 是       | -                             |
| skillIds       | 技能ID列表       | JSON文字  | -    | 是       | JSON陣列格式                   |
| coin           | 金幣獎勵         | 數字      | -    | 否       | 可能為空                       |
| coinVariable   | 隨機金幣獎勵     | 數字      | -    | 否       | 可能為空                       |
| itemId         | 第一個獎勵物品ID | 數字      | -    | 否       | 可能為空                       |
| itemCount      | 第一個獎勵物品數量 | 數字    | -    | 否       | 當itemId存在時返回             |
| items          | 物品獎勵列表     | JSON文字  | -    | 是       | JSON陣列格式                   |
| words          | 完成勵志語       | 文字      | -    | 否       | 可能為空                       |
| categoryId     | 分類ID           | 數字      | -    | 否       | 可能為空                       |
| order          | 排序             | 數字      | -    | 是       | -                              |
| name_extended  | 副檔名稱         | 文字      | -    | 是       | 與name相同                     |
| subTasks       | 子任務列表       | JSON文字  | -    | 是       | JSON陣列格式,見下文說明        |

**子任務(subTasks)欄位說明:**

`subTasks` 欄位是一個JSON陣列,每個元素包含以下欄位:

- `id`: 子任務ID
- `gid`: 子任務組ID
- `todo`: 子任務內容
- `status`: 子任務狀態(0=未完成, 1=已完成)
- `remindTime`: 提醒時間(Unix時間戳,毫秒)
- `exp`: 經驗值獎勵
- `coin`: 金幣獎勵
- `coinVariable`: 隨機金幣獎勵
- `items`: 物品獎勵列表
- `order`: 排序
- `autoUseItem`: 是否自動使用物品

<br/>

### 查詢屬性狀態 :id=query_skill

!> 此處的功能是用於搭配自動化工具/二次開發的

**方法名：**query_skill

**說明：**查詢指定技能的基礎資訊、原始排序欄位，以及等級/經驗值資料。
...
| 引數                 | 含義                         | 取值   | 示例  | 是否必須 | 備註 |
| -------------------- | ---------------------------- | ------ | ----- | -------- | ---- |
| id                   | 技能ID                       | 數字   | 1     | 是       | `query_skill` 於 v1.103.0+ 增加返回 |
| name                 | 屬性名稱                     | 字串 | 力量  | 是       | -    |
| order                | 原始排序值                   | 數字   | 20    | 是       | v1.103.0+ 新增；`orderInCategory` |
| group_id             | 技能組ID                     | 數字   | 10    | 是       | v1.103.0+ 新增；未分組時返回 `0` |
| status               | 狀態                         | 數字   | 0     | 是       | v1.103.0+ 新增；`0` = 正常，`1` = 隱藏 |
| level                | 等級                         | 數字   | 10    | 是       | -    |
...
### 查詢技能組 :id=query_skill_group

需要 v1.103.0+

**方法名：**query_skill_group

**說明：**查詢單個技能組，並返回原始排序值與摺疊狀態。

**示例：**

- 查詢技能組：[lifeup://api/query_skill_group?id=10](lifeup://api/query_skill_group?id=10)

| 引數 | 含義 | 取值 | 示例 | 是否必須 | 備註 |
| ---- | ---- | ---- | ---- | -------- | ---- |
| id | 技能組ID | 大於 0 的數字 | 10 | 是 | - |
...
| ---- | ---- | ---- | ---- | -------- | ---- |
| id | 技能組ID | 數字 | 10 | 是 | - |
| content | 技能組名稱 | 字串 | 戰鬥 | 是 | - |
| order | 原始排序值 | 數字 | 20 | 是 | `orderInCategory` |
| collapsed | 摺疊狀態 | 字串 | true | 是 | 以 `true` / `false` 文字返回 |
...
**說明：**當普通商品使用成功，或透過商品使用流程觸發的簡易合成成功時傳送。
...
> [!NOTE]
> 該廣播事件於 v1.102.8 版本釋出。

**名稱：**app.lifeup.synthesis.complete
...
| ------------ | ---------------------------- | ----------------------------------------------------- |
| formula_id   | 配方id                       | 1                                                     |
| formula_name | 配方名稱                     | 工具箱配方                                            |
| times        | 本次執行次數                 | 3                                                     |
| input_count  | 輸入材料條目數               | 2                                                     |
| output_count | 產物條目數                   | 1                                                     |
| inputs_json  | 本次總消耗材料的 JSON 陣列   | [{"item_id":7,"name":"木材","amount":6}]              |
| outputs_json | 本次總產出結果的 JSON 陣列   | [{"item_id":9,"name":"工具箱","amount":3}]            |

**注意：**

- 僅在配方合成成功後傳送。
- 材料不足、配方不存在、資料庫儲存失敗，或關閉`廣播事件`時，都不會發送該事件。
- 一次 API 呼叫只發送一條事件，即使 `times > 1` 也不會拆分成多條。
- `inputs_json` 和 `outputs_json` 中每個條目都使用如下結構：
...
- 其中 `amount` 表示**本次執行的總消耗 / 總產出**，不是單次配方量。
...
?> 該廣播事件於 v1.101.0 版本釋出，提供更豐富的事件資料。

**名稱：**

- 開始：app.lifeup.pomodoro.start
- 暫停：app.lifeup.pomodoro.pause (v1.101.0 新增)
- 停止：app.lifeup.pomodoro.stop
- 完成（正常結束）：app.lifeup.pomodoro.complete

**說明：**當番茄鍾啟動、暫停、停止或完成時，系統會發送相應的廣播事件，攜帶任務資訊、計時狀態等詳細資料。

**返回值：**

| 引數               | 含義                        | 示例                | 備註                                                           |
| ------------------ | --------------------------- | ------------------- | -------------------------------------------------------------  |
| task_id            | 任務ID                      | 1                   | 可選，僅當番茄鐘關聯了任務時存在                                 |
| task_gid           | 任務組ID                    | 1                   | 可選，僅當番茄鐘關聯了任務時存在                                 |
| name               | 任務名稱                    | 學習英語             | 番茄鐘關聯的任務名稱或自定義名稱                                |
| service_type       | 服務型別                    | 0                   | 0=專注，1=短休息，2=長休息                                     |
| service_type_label | 服務型別標籤                | 專注                | 本地化的服務型別文字                                            |
| duration           | 總時長（毫秒）              | 1500000             | 專注或休息的總時長                                              |
| remaining          | 剩餘時長（毫秒）            | 900000              | 當前剩餘的時長                                                  |
| elapsed            | 已用時長（毫秒）            | 600000              | 已經過的時長                                                    |
| start              | 開始時間                    | 1639123456789       | Unix 時間戳（毫秒）                                                |
| event_time         | 事件觸發時間                | 1639123456789       | Unix 時間戳（毫秒）                                                |
| reason             | 停止原因                    | user                | 僅 stop 事件有此欄位，可能值：manual, cancel, complete, auto       |

**停止原因說明：**

`reason` 引數僅在 `app.lifeup.pomodoro.stop` 事件中存在，表示番茄鐘停止的原因：

- `manual`: 使用者手動停止
- `cancel`: 使用者取消
- `complete`: 正常完成（注：完成時也會同時觸發 `app.lifeup.pomodoro.complete` 事件）
- `auto`: 自動停止（如任務刪除等）

### 正計時生命週期 :id=broadcast_positive_timing

> [!NOTE]
> 該組事件對應“正計時”功能，不同於上面的番茄鍾倒計時事件。使用前同樣需要先在 `設定`→`實驗`→`開發者模式`→`廣播事件` 中啟用廣播事件。

**名稱：**

- 開始：app.lifeup.timing.start
- 暫停：app.lifeup.timing.pause
- 完成：app.lifeup.timing.complete
- 放棄：app.lifeup.timing.abandon

**說明：**當正計時開始、暫停、完成或主動放棄時，系統會發送對應廣播。`complete` 表示本次正計時正常完成並寫入記錄，`abandon` 表示本次正計時被主動停止或放棄。

**返回值：**

| 引數    | 含義               | 示例          | 備註                                     |
| ------- | ------------------ | ------------- | ---------------------------------------- |
| task_id | 任務ID             | 1             | 可選，僅當正計時關聯了任務時存在         |
| name    | 任務名稱           | 學習英語      | 正計時關聯的任務名稱或自定義名稱         |
| time    | 已累計時長（毫秒） | 600000        | 當前正計時已累計的總時長                 |
| start   | 開始時間           | 1639123456789 | Unix 時間戳（毫秒）                      |
| end     | 結束時間           | 1639127056789 | 僅 `complete` / `abandon` 事件中存在     |
