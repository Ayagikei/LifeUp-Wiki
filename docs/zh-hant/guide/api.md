...
### 技能 ContentProvider 查詢 :id=skill_content_provider

需要 v1.103.0+

如果需要列表或結構化查詢，可以直接讀取以下 ContentProvider URI：
...
| desc | 描述 | 字串 | 可能為空 |
| icon | 圖示 URI | 字串 | 可能為空 |
| order | 原始排序值 | 數字 | `orderInCategory` |
| group_id | 技能組ID | 數字 / null | 未分組時可能為空 |
| color | 顏色 | 數字 / null | 可能為空 |
| exp | 經驗值 | 數字 | - |
| level | 當前等級 | 數字 | - |
| until_next_level_exp | 升到下一級所需經驗值 | 數字 | - |
| current_level_exp | 當前等級內已獲得經驗值 | 數字 | - |
| type | 技能型別 | 數字 | - |
| status | 技能狀態 | 數字 | `0` = 正常，`1` = 隱藏 |
...
| content | 技能組名稱 | 字串 | - |
| order | 原始排序值 | 數字 | `orderInCategory` |
| collapsed | 摺疊狀態 | 字串 | 以 `true` / `false` 文字返回 |
...
| start              | 開始時間                    | 1639123456789       | Unix時間戳(毫秒)                                                |
| event_time         | 事件觸發時間                | 1639123456789       | Unix時間戳(毫秒)                                                |
| reason             | 停止原因                    | user                | 僅stop事件有此欄位,可能值: manual, cancel, complete, auto       |
