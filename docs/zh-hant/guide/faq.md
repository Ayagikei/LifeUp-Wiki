...
?>  如果您有緊急問題需要查詢，請先快速瀏覽本節，不影響您快速瞭解應用核心功能。

*《人升》是一款與衆不同的獨立應用，它可能和您以往使用過的任何應用都不太一樣。*
...
- **零廣告，純粹體驗，卻面臨持續運營成本**
   ...
   《人升》沒有廣告，這意味著我們無法透過廣告獲取任何收入。但同時，爲了保證應用的正常執行，我們必須持續支出伺服器和物件儲存等服務的費用。

   > 一般來說，包含廣告的應用，每次曝光和點選都能帶來可觀的收入，這些收入可以用來維護市場口碑和客戶服務團隊。但《人升》沒有這些，我們完全依靠使用者的喜愛和支援來維持。

- **【無廣告】和【永久使用】本應是會員權益，但考慮到目前我們控制住了維護成本，決定開放給所有試用使用者。**

    雖然【無廣告】和【永久使用】嚴格意義上屬於會員權益，但考慮到目前的維護成本可控，我們決定將其作為福利開放給所有試用使用者。但這並不代表我們可以無限制地承擔風險。爲了保障全體會員權益和應用的長期發展，我們保留以下權利：

    -   在應用出現虧損、或虧損加劇、或運營成本難以覆蓋的情況下，我們可能會取消試用使用者的【無廣告】權益 （此調整不影響會員使用者和Pro版本）。
    -   在應用出現虧損、或虧損加劇、或運營成本難以覆蓋的情況下，我們可能會取消試用使用者的【無限期試用】權益（此調整不影響會員使用者和 Pro 版本）。

    > 我們會盡力避免這些情況出現，但我們希望使用者能夠理解我們作為獨立開發者的不易。

#### **獨立團隊，業餘維護**

- **我們是一支小團隊，利用業餘時間維護、開發和測試應用**

    《人升》不是商業產品，它誕生於業餘時間。能夠在各大應用市場上架，已經是一個不小的挑戰。目前，只有兩位開發者在業餘時間維護這款應用，我們要溝通協作，共同承擔包括QQ群諮詢在內的所有工作。

   > 獨立開發者的關注點：
   >
   >  1.  不要讓自己成為客服，要讓文件和應用指引承擔客服的職責。——（摘自V2EX某文章的回覆）
   >  2.  獨立開發者更能瞭解使用者的真實需求，並提供針對性的服務 —— （摘自某獨立應用開發者的分享）

   在人手有限的情況下，我們必須努力平衡使用者反饋、功能開發、文件維護等各項工作。如果過於專注於使用者反饋，勢必會影響應用的開發進度，從而導致使用者對新功能期待的增加，最終形成惡性循環。事實上，我們已經收集了許多具有建設性的建議，但因為時間有限，許多功能仍在排隊等待開發。所以，一些細緻的建議能更好地幫到我們！

    **我們誠摯地邀請小夥伴們加入到《人升》的社羣共建中來，一同維護QQ諮詢群，完善文件內容等等，期待您的加入！**

#### **資料安全，本地儲存**

-  **離線儲存，資訊更安全**
   
   除了“世界模組”以外的所有資料資訊，幾乎都不會透過伺服器進行數據傳輸, 你的資訊由你掌握，所以你也需要負責資料備份等工作。

   > 微信的聊天記錄也是離線儲存的，需要你手動備份，這和我們是相同的理念。
   >
   >  即使微信目前想要推出雲端儲存，也計劃推出付費服務 ([微信擬推出聊天記錄付費雲端儲存服務](https://www.sohu.com/a/487723432_507132)）。
   >
    >  作為一款獨立應用，《人升》目前真的難以實現雲端儲存。

#### **買斷制，情懷之選**

-  **我們選擇了買斷制，一種對使用者負責的方式**
    
    誠然，買斷制模式已經很難滿足軟件開發高昂的成本和持續投入的需求， ([為什麼我們還是不喜歡訂閱制？](https://www.ifanr.com/app/1248797)), 但我們仍然想提供一款高品質、能夠讓使用者一次付費就能擁有其使用權的應用（並且基於現狀，我們真的做不到雲端儲存）。所以我們選擇了買斷制，但這對於應用的持續維護和開發確實非常不利。

  但因國內應用市場基本不提供買斷服務，我們又不想閹割應用功能，纔有了現在的會員機制（免費使用者可以使用 95% 的功能）。
    
  >   但當前會員捐贈收入遠不如海外市場買斷制的收入，用愛發電的模式難以長期發展。
      
  >   爲了更好的生存，我們未來可能會調整會員策略（新功能優先給會員，會員內測週期延長等），希望能夠得到理解。
...
[🔶單個任務分配不同屬性的不同經驗值？](/guide/faq?id=🔶單個任務分配不同屬性的不同經驗值？)
...
[🔶商品圖片無法顯示或丟失怎麼辦？](/guide/faq?id=🔶商品圖片無法顯示或丟失怎麼辦？)
...
[🔶為什麼沒有在App Store上架？有沒有iOS版本？](/guide/faq?id=🔶為什麼沒有在app-store上架？有沒有ios版本？)
...
> - **由於群人數有限，我們可能會不時清理非活躍使用者**，以便新使用者可以加入討論。但這種情況，如果你遇到了新的問題的話，歡迎隨時重新加入。你也可以選擇加入不限人數的 QQ 頻道作為替代。
> - 討論需要遵循友善、尊重的前提，對於不友善的使用者或者行為，我們可能會永久禁止加入相關社群。
> - 這是討論群，而非會員售後群。所以上述各點，**無論是否會員，都一視同仁。**
...
> 由於 QQ 群、QQ 頻道是面向全員的，而非會員售後群。無論是否會員，以上各點都一視同仁。
...
##### 🔶為什麼沒有在App Store上架？有沒有iOS版本？

《人升》暫時沒有 iOS 版本，《人升》是使用純 Android 技術進行開發的原生 Android 應用，沒辦法簡單遷移到 iOS 版本。

**我們已經推出了專門為 iOS 使用者設計的效率應用 [ulives](https://apps.apple.com/us/app/ulives-gamify-your-tasks/id6743084261)！它繼承了人升的核心理念，並針對蘋果生態系統進行了最佳化。**
