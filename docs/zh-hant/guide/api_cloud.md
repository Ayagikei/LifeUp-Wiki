...
《雲人升》是《人升》API 開發的基礎套件之一。它把手機變成 **API 橋樑**：你可以從**電腦或其他裝置**控制人升、做**二次擴充開發**（桌面端、網頁、自動化腳本），也可以搭配 **AI Agent** 使用。

### 典型使用場景

| 場景 | 適合誰 | 一句話 | 了解更多 |
| --- | --- | --- | --- |
| **HTTP 服務 + 自研** | 開發者、自動化愛好者 | 用 Python、網頁、腳本跨裝置調 API、查資料，無需 Android 開發經驗 | [HTTP 介面](#http-介面定義)、[桌面端](guide/api_desktop.md) |
| **二維碼掃描** | 想與現實行為聯動的人 | 列印二維碼，掃碼完成任務打卡、計時、彈窗，或打開網頁 / 其他 App | [二維碼掃描](#二維碼掃描) |
...
雲人升首頁右上角的 **掃碼** 按鈕，可讀取二維碼中的連結並立即打開——無需手動輸入。

掃碼**不限於人升 API**：`lifeup://api/…`、網頁 `https://…`、以及其他 App 的 URL Scheme（如 `weixin://`）均可識別，由 Android 系統打開對應目標。

### 怎麼用

1. 在《人升》或 Wiki [API 列表](guide/api.md) 中找到需要的 URL（例如完成任務、開始番茄鐘、彈出購買/使用商品對話框，或打開外部網頁）。

2. 用任意二維碼生成器把完整 URL 轉成二維碼（可列印貼在桌面、牆面、裝置旁）。

3. 打開《雲人升》，點 **掃碼**（旁邊的 **?** 可查看簡要說明），對準二維碼即可。

### 可以做什麼

| 場景 | 示例（示意） |
| --- | --- |
| 番茄鐘打卡 | `lifeup://api/pomodoro?...` 開始/結束計時 |
| 任務打卡 | `lifeup://api/complete?name=...` 完成任務 |
| 商店互動 | `lifeup://api/goto?page=shop` 或購買/使用彈窗 |
| 獎勵/懲罰 | `lifeup://api/reward?...` / `lifeup://api/penalty?...` |
| 打開網頁小遊戲 | `https://wiki.lifeupapp.fun/zh-hant/example/wordle/index.html` |
| 跳轉其他 App | `weixin://`、`intent://` 等已安裝 App 的 Scheme |
| 自訂流程 | 任意可掃描 URL 組合 |

### 與現實互動

二維碼可以把**現實位置或實體動作**映射到 App 裡的遊戲化行為，例如：
