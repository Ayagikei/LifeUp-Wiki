...
### 小部件新增方式（各系統入口） :id=widget-entrypoints

目前各大系統都推出了自己的特殊小部件，並將安卓通用小部件的新增入口隱藏起來了；同時大部分系統也會限制 App 直接請求新增小部件，需要在系統桌面中手動翻找入口。

省流（大部分新版本系統通用）：
1. 在桌面長按或雙指捏合，進入桌面編輯模式
2. 找到系統特殊小部件的新增頁面（命名因系統而異，如華為：服務卡片、vivo：原子元件、OPPO：卡片）
3. 進入系統小部件列表後，快速滑到最下方，找到小入口（如“視窗小工具”“應用掛件”“外掛”）
4. 點選小入口進入安卓通用小部件列表，在其中找到“人升”並新增
5. （可選）安卓小部件有最小尺寸限制，但可自行放大：新增後原地長按並鬆手，拖動右下角等控點即可

| 系統/品牌 | 系統特殊小部件入口 | 安卓通用小部件入口（通常在列表底部） | 影片教程 |
| --- | --- | --- | --- |
| 小米/紅米（MIUI/澎湃 OS） | 系統小部件入口（名稱因版本略有差異） | 安卓通用小部件入口（通常在列表底部） | https://www.bilibili.com/video/BV17W4y1s7dL |
| 華為/榮耀（HarmonyOS/EMUI） | 服務卡片 | 視窗小工具 | https://www.bilibili.com/video/BV1RYZMYvE7X |
| vivo/iQOO | 原子元件 | 應用掛件 | https://www.bilibili.com/video/BV1iu411R7VK |
| OPPO/Realme/一加 | 卡片 | 外掛 | https://www.bilibili.com/video/BV1Qr76ztEmF/ |
...
人升僅對雙框架鴻蒙系統（HarmonyOS 4.3.0 及以下）相容；鴻蒙 5 及以上可透過「卓易通」執行 Android 版，或改用 **[ulives 鴻蒙原生版](https://ulives.lifeupapp.net/)**（AppGallery，限時免費公測）。

完整說明與限制請見：[鴻蒙系統相容性說明](guide/harmonyos-compatibility.md)。
