<h1 align="center" padding="100">進入 LifeUp 的世界</h1>



> [現已推出繁體中文文檔，若有需要，請點擊這裡跳轉。](https://wiki.lifeupapp.fun/zh-hant/#/guide/hello_lifeup)

>  提示：如果你是透過 Google Play 購買的 LifeUp。由於我們和 Google 都深知一款應用難以滿足所有人，只要你在購買後一段時間內解除安裝 LifeUp 即可獲得退款。如果你已經超過了退款期限，仍可聯絡我們的郵箱獲得幫助 kei.ayagi@gmail.com 。
...
`人升`是一款清單待辦事項型別的應用，其重複機制足以支撐習慣養成。

以及能夠滿足日常的簡易待辦登記、習慣統計等功能。

但`人升`並不是一款日程管理、專案管理型應用，它並不適合用於繁雜的日程管理工作。

這之間的差異，相當於系統自帶的`待辦`軟體和`日曆`軟體。
...
但這也意味著你也確實需要珍重應用內的 ` 經驗值 `、` 等級 `、` 金幣 ` 等概念，而不能隨意作弊欺騙自己。

這是一款`記錄`軟體，待辦事項、習慣類應用也皆如此。如果你需要所謂的「督促」，更建議找其他小夥伴一起進步，而不是尋找一款軟體。

#### # 獨立開發

**獨立**

這是一款由 3 人在**業餘時間**獨立開發的應用，並由使用者自主宣傳、建設社羣而成的小應用。 因此我們不會也沒有資源聚焦於一些商業化砸錢功能，如 ` 社交 `、` 伺服器同步 `、` 任務分派 `。
如果你需要上述的功能，可能需要另尋其他 APP 使用或與 ` 人升 ` 相輔使用。

> 如果你需要大而全的待辦+日程管理應用，可以使用《滴答清單（TickTick）》；
>
> 如果你想要的是支援多端同步的簡潔待辦事項，可以使用《微軟 ToDo》、《Google Tasks》；
>
> 如果你想要的是副本合作遊戲化待辦事項，可以嘗試《Habitca》。
...
<p align="center">
 ...
 [1.80.0 - 開箱 (lifeupapp.fun)](features/180)
...
*LifeUp*的番茄鍾屬於次要模組，以功能簡潔、搭配其他模組為主，我們不會為其增加繁雜的功能如 `白噪聲`、`鎖機`，具體原因可參見常見問題。但目前番茄鍾仍有大量改進空間，我們會繼續完善統計等功能。
...
感想功能屬於 *LifeUp* 的次級功能模組，僅提供簡短的文字、圖片記錄功能。

!> 每篇感想目前限制 500 字元和 9 張圖片。
